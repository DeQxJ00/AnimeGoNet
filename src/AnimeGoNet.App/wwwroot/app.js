...
import { clearStoredWebUiAccessKey, createAuthenticatedFetch, loadWebUiAccessKey, sha256LowerHex, storeWebUiAccessKey, } from "./webui-auth.js";
...
const webUiAccessKeyRemember = element("#webui-access-key-remember");
const webUiAccessKeyMessage = element("#webui-access-key-message");
const webUiAccessKeySubmit = element("#webui-access-key-submit");
let pendingWebUiAccessKeyRequest = null;
function applyWebUiAccessKey(accessKeyHash, remember) {
...
function requestWebUiAccessKey(message) {
    if (pendingWebUiAccessKeyRequest)
        return pendingWebUiAccessKeyRequest.promise;
    ...
    if (!webUiAccessKeyDialog.open)
        webUiAccessKeyDialog.showModal();
    queueMicrotask(() => webUiAccessKeyInput.focus());
    return promise;
}
function finishWebUiAccessKeyRequest(value) {
    const pending = pendingWebUiAccessKeyRequest;
    pendingWebUiAccessKeyRequest = null;
    if (webUiAccessKeyDialog.open)
        webUiAccessKeyDialog.close();
    pending?.resolve(value);
}
async function validateEnteredWebUiAccessKey() {
...
const movieFileDialog = element("#movie-file-dialog");
...
let movieLibraryPageSize = 24;
let movieLibrarySearch = "";
let movieLibrarySort = "last_updated";
let movieLibraryDirection = "desc";
...
async function saveExternalPluginConfiguration(event, configuration, configurationRevision, enabled, argsInput, message) {
    ...
    try {
        ...
        const response = await authenticatedFetch(`/api/v1/plugins/${encodeURIComponent(configuration.id)}/configuration`, {
            method: "PUT",
...
async function deleteExternalPluginConfiguration(configuration, configurationRevision, button) {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/plugins/${encodeURIComponent(configuration.id)}/configuration?expected_revision=${configurationRevision}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        await loadStatus();
    }
...
async function loadBangumiArchiveUsage(silent = false) {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/data-update/archive-usage?${query.toString()}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
...
let anidbCachePageSize = 25;
let anidbCacheTotalItems = 0;
let anidbCacheBusy = false;
function anidbCacheMessage(text, alert = false) {
    const row = document.createElement("tr");
    const cell = document.createElement("td");
    cell.colSpan = 4;
...
function renderAnidbTitles(page) {
    const target = element("#anidb-cache-list");
    ...
    const totalPages = Math.max(1, Math.ceil(page.total_items / page.page_size));
    element("#anidb-cache-page").textContent =
        `第 ${page.page} / ${totalPages} 页 · 共 ${page.total_items} 条`;
    element("#anidb-cache-previous").disabled = page.page <= 1;
    element("#anidb-cache-next").disabled = page.page >= totalPages;
}
async function loadAnidbTitles() {
    const aidValue = element("#anidb-cache-aid").valueAsNumber;
    const queryValue = element("#anidb-cache-query").value.trim();
    const query = new URLSearchParams({
        page: String(anidbCachePage),
        page_size: String(anidbCachePageSize),
    });
    if (Number.isInteger(aidValue) && aidValue > 0)
        query.set("aid", String(aidValue));
    else if (queryValue)
        query.set("query", queryValue);
    try {
        const response = await authenticatedFetch(`/api/v1/cache/anidb/titles?${query.toString()}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        renderAnidbTitles(await response.json());
    }
    catch (error) {
        element("#anidb-cache-list").replaceChildren(anidbCacheMessage(errorMessage(error, "AniDB 标题读取失败"), true));
    }
}
async function loadAnidbTitleCache(silent = false) {
    const message = element("#anidb-cache-status");
    if (!silent)
        message.textContent = "正在读取 AniDB 缓存状态…";
    try {
        const response = await authenticatedFetch("/api/v1/cache/anidb", { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const status = await response.json();
        element("#anidb-cache-interval").value = String(status.refresh_interval_hours);
        ...
        element("#anidb-cache-summary").replaceChildren(configurationCard("缓存规模", [
...
async function refreshAnidbTitleCache() {
    if (anidbCacheBusy)
        return;
    anidbCacheBusy = true;
    const button = element("#anidb-cache-refresh");
    const message = element("#anidb-cache-status");
    button.disabled = true;
...
async function saveAnidbCacheSettings(event) {
    event.preventDefault();
    const input = element("#anidb-cache-interval");
    const status = element("#anidb-cache-settings-status");
...
function renderLiveLogs() {
    ...
    element("#live-log-total").textContent = String(liveLogEntries.length);
...
function setLiveLogDomain(domain) {
    liveLogDomain = domain;
    document.querySelectorAll("[data-log-domain]").forEach(button => {
        button.classList.toggle("active", button.dataset.logDomain === domain);
    });
    renderLiveLogs();
}
function aiLogResultLabel(value) {
    return {
        matched: "成功",
        not_matched: "未匹配",
        error: "调用 / 验证错误",
...
function aiLogValidatedEpisodes(item) {
    if (item.validated_episodes.length === 0)
        return "未通过最终 TMDB Episode 验证";
    return item.validated_episodes.map(episode => `TMDB ${episode.tmdb_series_id} · S${String(episode.season_number).padStart(2, "0")}`
        + `E${String(episode.episode_number).padStart(3, "0")}`
        + `${episode.episode_name ? ` · ${episode.episode_name}` : ""}`).join("；");
}
function renderAiInvocationLogSummary(result) {
...
function aiDebugCodeBlock(value, className = "ai-debug-payload") {
...
function aiDebugDisclosure(title, value, open = false) {
    const details = document.createElement("details");
...
function renderAiDebugDocument(documentValue) {
    ...
    metadata.append(liveLogDetail("Trace / Run", `${chain.trace_id} / ${textOrDash(chain.run_id)}`), liveLogDetail("任务 ID", textOrDash(chain.task_id)), liveLogDetail("模型", `${chain.model} · ${chain.api_mode} · ${chain.prompt_version}`), liveLogDetail("时间", `${dataUpdateTime(chain.started_at_utc)} → ${dataUpdateTime(chain.completed_at_utc)}`), liveLogDetail("最终状态", chain.failure_code ?? "本地验证已执行"), liveLogDetail("记录格式", `v${documentValue.format_version}`));
    ...
    if (preAi) {
        ...
        evidence.append(liveLogDetail("Torrent 发布时间", textOrDash(preAi.torrent_published_at)), liveLogDetail("Bangumi EP 候选", textOrDash(preAi.bangumi_episode_candidate)), liveLogDetail("发布时间优先", preAi.use_bangumi_pub_date_first ? "启用" : "未启用"), liveLogDetail("发布时间证据", `${preAi.publication_result}${preAi.publication_error_code ? ` · ${preAi.publication_error_code}` : ""}`), liveLogDetail("模型外锁定 TMDB", preAi.expected_tmdb_series_id === null
            ? "未锁定"
            : `${preAi.expected_tmdb_series_id} / S${String(preAi.expected_season_number ?? 0).padStart(2, "0")}`), liveLogDetail("已尝试 TMDB 搜索词", preAi.attempted_tmdb_search_titles.length === 0
            ? "无"
    ...
    promptSection.append(promptTitle, aiDebugDisclosure("原始 Prompt 模板", chain.prompt_template), aiDebugDisclosure("最终渲染 Prompt（实际发给模型）", chain.rendered_prompt, true));
    ...
    for (const exchange of chain.exchanges) {
        ...
        if (exchange.request_body !== null)
            item.append(aiDebugDisclosure("请求 Body", exchange.request_body));
        if (exchange.response_body !== null)
            item.append(aiDebugDisclosure("响应 Body", exchange.response_body));
        if (exchange.error !== null)
            item.append(aiDebugDisclosure("错误", exchange.error, true));
...
async function openAiDebugChain(item) {
    activeAiDebugRunId = item.run_id;
    element("#ai-debug-dialog-title").textContent = `AI Debug · ${item.title}`;
    element("#ai-debug-status").textContent = "正在读取完整链路…";
    element("#ai-debug-content").replaceChildren();
    element("#ai-debug-delete").disabled = true;
    aiDebugDialog.showModal();
    try {
        const documentValue = await api.get(`/api/v1/logs/ai-invocations/${encodeURIComponent(item.run_id)}/debug`);
        if (activeAiDebugRunId !== item.run_id)
            return;
        renderAiDebugDocument(documentValue);
        element("#ai-debug-status").textContent =
            "本机敏感调试记录：包含任务标题、文件名、Prompt 与 AI/MCP Body；不包含 API Key、Cookie、passkey 或 Authorization Header。";
        element("#ai-debug-delete").disabled = false;
    }
    catch (error) {
        if (activeAiDebugRunId !== item.run_id)
            return;
...
async function deleteActiveAiDebugChain() {
    if (activeAiDebugRunId === null)
        return;
    if (!window.confirm("只删除这一次 AI Debug 完整链路文件？任务、匹配结果和普通 AI 调用日志不会删除。"))
        return;
    const runId = activeAiDebugRunId;
    const button = element("#ai-debug-delete");
    button.disabled = true;
    try {
        await api.delete(`/api/v1/logs/ai-invocations/${encodeURIComponent(runId)}/debug`);
        aiDebugDialog.close();
...
function renderAiInvocationLogs(result) {
    const list = element("#ai-log-list");
    ...
    element("#ai-log-page-label").textContent =
        `第 ${result.page} / ${lastPage} 页 · ${result.total_items.toLocaleString("zh-CN")} 条`;
    element("#ai-log-previous").disabled = result.page <= 1;
    element("#ai-log-next").disabled = result.page >= lastPage;
    element("#ai-log-status").textContent =
        `已读取 ${result.items.length} 条；汇总基于当前全部筛选结果。`;
    renderAiInvocationLogSummary(result);
}
async function loadAiInvocationLogs() {
    const sequence = ++aiLogRequestSequence;
    const list = element("#ai-log-list");
    setRegionState(list, "loading");
    element("#ai-log-status").textContent = "正在读取 AI 调用记录…";
    const query = new URLSearchParams({
    ...
    for (const [key, value] of values)
        if (value)
            query.set(key, value);
    try {
        const result = await api.get(`/api/v1/logs/ai-invocations?${query}`);
        if (sequence !== aiLogRequestSequence)
            return;
        ...
        element("#ai-log-status").textContent = "AI 调用日志读取失败。";
    }
}
...
function syncDownloadFilterControls() {
...
function externalMediaImportReason(value) {
    const labels = {
...
function renderMovieLibraryPage(page) {
    const list = element("#movie-library-list");
    const pageCount = Math.max(1, Math.ceil(page.total_items / page.page_size));
    element("#movie-library-status").textContent =
        `${page.total_items} 部电影 · ${librarySortLabel(page.sort)} · `
            + (page.direction === "asc" ? "升序" : "降序")
            + (movieLibrarySearch ? ` · 搜索“${movieLibrarySearch}”` : "");
    element("#movie-library-page-label").textContent =
        `第 ${page.page} / ${pageCount} 页`;
    element("#movie-library-previous").disabled = page.page <= 1;
    element("#movie-library-next").disabled = page.page >= pageCount;
    if (page.items.length === 0) {
        renderRegionMessage(list, "empty", movieLibrarySearch
    ...
    renderRegionContent(list, ...page.items.map((item) => {
        ...
        deleteMenuLabel.addEventListener("click", () => {
            requestAnimationFrame(() => {
                if (!deleteOptions.matches(":popover-open"))
                    return;
                ...
                const left = Math.min(Math.max(margin, trigger.right - width), Math.max(margin, window.innerWidth - width - margin));
        ...
        deleteOptions.addEventListener("toggle", () => {
            deleteMenuLabel.setAttribute("aria-expanded", String(deleteOptions.matches(":popover-open")));
        });
...
function movieFileRoleLabel(role) {
    if (role === "movie")
        return "Movie 主文件";
    if (role === "extras")
        return "Movie Extras";
...
async function openMovieFileList(item) {
    element("#movie-file-dialog-title").textContent = `${item.title} · 文件清单`;
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/library/movies/${item.tmdb_movie_id}/files`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        ...
        renderRegionMessage(container, "error", `电影文件读取失败：${errorMessage(error, "未知错误")}`);
    }
}
async function forceDeleteOrphanMovie(item, detail, button) {
    const confirmation = window.prompt(`强制删除会移除 ${item.title} 的孤立完成记录、本地 Movie 投影，`
        + `以及上方列出的 ${detail.files.length} 个 Movie 库文件。不会删除 Movie 根目录之外的文件。\n\n`
        + `请输入 TMDB Movie ID ${item.tmdb_movie_id} 确认：`);
    if (confirmation?.trim() !== String(item.tmdb_movie_id))
        return;
    const message = element("#movie-file-dialog-message");
    button.disabled = true;
    message.textContent = "正在校验孤立状态并强制删除 Movie 文件与记录…";
    try {
        const response = await authenticatedFetch(`/api/v1/library/movies/${item.tmdb_movie_id}/force-delete`, {
            method: "POST",
            headers: jsonRequestHeaders(),
            body: JSON.stringify({
                expected_revision: detail.resource_revision,
...
async function loadMovieLibrary(background = false) {
    ...
    if (!background) {
        setRegionState(list, "loading");
        element("#movie-library-status").textContent = "正在读取电影库…";
    }
    const query = new URLSearchParams({
    ...
    if (movieLibrarySearch)
        query.set("search", movieLibrarySearch);
    try {
        const response = await authenticatedFetch(`/api/v1/library/movies?${query}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const page = await response.json();
        if (sequence !== movieLibraryRequestSequence)
            return;
        ...
        if (sequence !== movieLibraryRequestSequence)
            return;
        if (!background) {
            renderRegionMessage(list, "error", `电影库读取失败：${errorMessage(error, "未知错误")}`);
            element("#movie-library-status").textContent = "电影库读取失败";
        }
    }
}
...
async function loadLibraryDetail(tmdbSeriesId, seasonNumber, focus = false) {
    ...
    if (externalImportDetailIdentity !== detailIdentity) {
        const importResult = element("#library-detail-external-import-result");
    ...
    element("#library-detail-action-status").textContent = "";
    try {
        const response = await authenticatedFetch(`/api/v1/library/seasons/${tmdbSeriesId}/${seasonNumber}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const detail = await response.json();
...
function selectedMikanSeasonCompletionWork() {
...
function selectedMikanSeasonCompletionGroups() {
    return Array.from(document.querySelectorAll("#mikan-season-completion-groups input[data-mikan-completion-group]:checked"))
...
function updateMikanSeasonCompletionSelectionStatus() {
    ...
    element("#mikan-season-completion-status").textContent =
        `字幕组 ${completion.previews.length} 个 · RSS ${completion.previews.reduce((sum, preview) => sum + preview.items.length, 0)} 条 · 已选 ${selected} 条 · ${offsets.join("；")}${failures}`;
}
function renderMikanSeasonCompletion(completion) {
...
function renderMikanSeasonCompletionGroups(discovery) {
    const groups = element("#mikan-season-completion-groups");
    groups.replaceChildren(...discovery.groups.map((group) => {
        const label = document.createElement("label");
        if (group.previously_used)
            label.classList.add("previously-used");
        const checkbox = document.createElement("input");
        checkbox.type = "checkbox";
        ...
        checkbox.addEventListener("change", () => {
            ...
            element("#mikan-season-completion-preview").disabled = count === 0;
            element("#mikan-season-completion-status").textContent = count === 0
                ? "请选择至少一个字幕组。"
                : `已选 ${count} 个字幕组；点击“预览已选字幕组”读取季度 RSS。`;
        });
        const text = document.createElement("span");
        text.textContent = `${group.name} · groupid ${group.groupid}${group.previously_used ? " · 已用" : ""}`;
        label.append(checkbox, text);
        return label;
    }));
    element("#mikan-season-completion-preview").disabled =
        selectedMikanSeasonCompletionGroups().length === 0;
}
async function discoverMikanSeasonCompletionGroups() {
    ...
    activeMikanSeasonCompletion = null;
    element("#mikan-season-completion-preview").disabled = true;
    element("#mikan-season-completion-confirm").disabled = true;
    element("#mikan-season-completion-items").replaceChildren();
    const groups = element("#mikan-season-completion-groups");
    ...
    try {
        const requestHeaders = new Headers(headers);
        requestHeaders.set("Content-Type", "application/json");
        const response = await authenticatedFetch(`/api/v1/library/seasons/${detail.tmdb_series_id}/${detail.tmdb_season_number}/mikan-completion/groups`, {
            method: "POST",
            headers: requestHeaders,
            body: JSON.stringify({
...
async function previewMikanSeasonCompletion() {
    ...
    if (!detail || !work || !work.discovery || groupIds.length === 0)
        return;
    const sequence = ++mikanSeasonCompletionRequestSequence;
    activeMikanSeasonCompletion = null;
    element("#mikan-season-completion-confirm").disabled = true;
    ...
    try {
        ...
        if (previews.length === 0)
            throw new Error(failures.join("；") || "所有字幕组 RSS 预览均失败");
...
function openMikanSeasonCompletion() {
    ...
    bindings.replaceChildren(...activeMikanSeasonWorks.map((work, index) => {
        ...
        option.textContent = `${work.source_profile_id} · mikanid ${work.mikanid} · ${known} · 最近 ${libraryDate(work.last_used_at_utc, true)}`;
        return option;
    }));
    element("#mikan-season-completion-title").textContent =
        `${detail.display_name} · ${detail.season_name} · EP 自动补完`;
    element("#mikan-season-completion-items").replaceChildren();
    element("#mikan-season-completion-groups").replaceChildren();
...
async function confirmMikanSeasonCompletion() {
    ...
    if (!detail || !completion)
        return;
    const selected = Array.from(document.querySelectorAll("#mikan-season-completion-items input[data-mikan-completion-candidate]:checked"));
    ...
    try {
        ...
        const settled = await Promise.allSettled(Array.from(selectedByGroup.entries()).map(async ([groupId, candidateIds]) => {
            const preview = completion.previews.find((item) => item.groupid === groupId);
            if (!preview)
                throw new Error(`groupid ${groupId} 缺少有效预览`);
        ...
        const alreadyKnown = items.filter((item) => ["already_ingested", "already_completed"].includes(item.status)).length;
        ...
        if (failures.length === 0) {
            ...
            element("#library-detail-action-status").textContent = `${summary}。`;
        }
        else {
            element("#mikan-season-completion-status").textContent =
...
async function refreshLibrarySeason() {
    ...
    const deleteContent = element("#library-detail-delete-content");
    ...
    try {
        const requestHeaders = new Headers(headers);
        requestHeaders.set("Content-Type", "application/json");
        const response = await authenticatedFetch(`/api/v1/library/seasons/${detail.tmdb_series_id}/${detail.tmdb_season_number}`, {
            method: "PUT",
            headers: requestHeaders,
...
async function importExternalMedia(scope) {
    const detail = activeLibraryDetail;
    ...
    if (!window.confirm(message))
        return;
    const globalButton = element("#library-external-import");
    const seasonButton = element("#library-detail-external-import");
    const refreshButton = element("#library-detail-refresh");
    const deleteContentButton = element("#library-detail-delete-content");
    const deleteButton = element("#library-detail-delete");
    const target = scope === "all"
        ? "#library-external-import-result"
        : "#library-detail-external-import-result";
    const resultContainer = element(target);
    if (scope === "season" && detail) {
...
function renderSubtitleImportCandidates() {
    const panel = element("#library-subtitle-import-panel");
    const list = element("#library-subtitle-import-list");
    const preview = element("#library-subtitle-file-preview-name");
    ...
    list.replaceChildren(...session.candidates.map((candidate) => {
        ...
        row.addEventListener("drop", (event) => {
            ...
            const source = list.querySelector(`[data-candidate-id="${CSS.escape(sourceId)}"]`);
            if (source && source !== row)
                list.insertBefore(source, row);
        });
        row.addEventListener("dragstart", (event) => event.dataTransfer?.setData("text/plain", candidate.id));
        const enabled = document.createElement("input");
        enabled.type = "checkbox";
        enabled.checked = candidate.selected_episode !== null;
    ...
    element("#library-subtitle-import-status").textContent =
        `${session.candidates.length} 个字幕文件 · 已默认勾选可识别 EP 的文件`;
}
async function importSubtitleArchive() {
    const detail = activeLibraryDetail;
    const fileInput = element("#library-subtitle-archive-file");
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/library/subtitle-archives/import?tmdbSeriesId=${detail.tmdb_series_id}&seasonNumber=${detail.tmdb_season_number}`, {
            method: "POST",
            headers: {
                ...headers,
        ...
        if (!response.ok)
            throw new Error(await responseError(response));
        activeSubtitleImport = await response.json();
        renderSubtitleImportCandidates();
    }
    catch (error) {
        activeSubtitleImport = null;
        element("#library-subtitle-import-panel").hidden = false;
...
async function confirmSubtitleArchiveImport() {
    if (!activeSubtitleImport)
        return;
    const detail = activeLibraryDetail;
    const list = element("#library-subtitle-import-list");
    const assignments = activeSubtitleImport.candidates.flatMap((candidate) => {
        const enabled = list.querySelector(`[data-subtitle-enabled="${CSS.escape(candidate.id)}"]`);
        if (!enabled?.checked)
            return [];
        const input = list.querySelector(`[data-subtitle-episode="${CSS.escape(candidate.id)}"]`);
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/library/subtitle-archives/${activeSubtitleImport.session_id}/confirm`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ assignments }) });
        if (!response.ok)
            throw new Error(await responseError(response));
        const result = await response.json();
...
async function deleteLibrarySeason() {
    ...
    try {
        ...
        const response = await authenticatedFetch(`/api/v1/library/seasons/${detail.tmdb_series_id}/${detail.tmdb_season_number}?${query}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        closeLibraryDetail();
...
function openLibraryContentDeletion() {
    if (!activeLibraryDetail)
        return;
    const detail = activeLibraryDetail;
    const status = element("#library-detail-action-status");
    ...
    const firstDelete = group.querySelector("button[data-library-delete-task]");
...
function closeLibraryDetail() {
    ...
    element("#library-detail-external-import-result").hidden = true;
...
async function restartRuntime() {
    if (!window.confirm("确定重启 AnimeGoNet？当前请求会返回后进程停止；请确认有服务管理器或手动方式负责重新启动。"))
        return;
    const status = element("#webui-authentication-status");
    const button = element("#webui-restart");
    button.disabled = true;
...
async function loadWebApiCompatibility() {
    ...
    const keyInput = element("#web-api-compatibility-access-key");
    const reload = element("#web-api-compatibility-reload");
...
async function saveWebApiCompatibility(event) {
    event.preventDefault();
    const status = element("#web-api-compatibility-status");
    const reload = element("#web-api-compatibility-reload");
    const save = element("#web-api-compatibility-save");
    const requestedKey = element("#web-api-compatibility-access-key").value;
...
async function loadU2WebApiCompatibility() {
    const status = element("#u2-web-api-status");
...
async function saveU2WebApiCompatibility(event) {
    event.preventDefault();
    const status = element("#u2-web-api-status");
    const reload = element("#u2-web-api-reload");
    const save = element("#u2-web-api-save");
    const requestedKey = element("#u2-web-api-access-key").value;
    if (!requestedKey) {
...
async function importConfigurationArchive() {
    ...
    try {
        const requestHeaders = new Headers(headers);
        requestHeaders.set("Content-Type", "application/json");
        const response = await authenticatedFetch(`/api/v1/configuration-archive/import?expected_sha256=${encodeURIComponent(preview.sha256)}`, { method: "POST", headers: requestHeaders, body: file });
        if (!response.ok)
            throw new Error(await responseError(response));
        const result = await response.json();
...
async function restoreConfigurationBackup(backup) {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/configuration-archive/backups/${encodeURIComponent(backup.id)}/restore`, { method: "POST", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const result = await response.json();
...
async function deleteConfigurationBackup(backup) {
    ...
    const status = element("#configuration-archive-status");
    try {
        const response = await authenticatedFetch(`/api/v1/configuration-archive/backups/${encodeURIComponent(backup.id)}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
...
function configurationRequest() {
    ...
    return {
        ...
        movie_save_path: element("#configuration-movie-save-path").value,
...
async function resetConfiguration() {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/config?expected_revision=${currentConfiguration.configuration_revision}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const saved = await response.json();
        await loadConfiguration();
...
function formatTmdbEpisodeNumbers(values) {
...
async function controlDownload(item, action, button) {
    ...
    try {
        const requestHeaders = new Headers(headers);
        requestHeaders.set("Content-Type", "application/json");
        const response = await authenticatedFetch(`/api/v1/downloads/${encodeURIComponent(item.job_id)}/${action}`, {
            method: "POST",
            headers: requestHeaders,
...
async function loadDownloadDetail(item, target, button) {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/downloads/${encodeURIComponent(item.job_id)}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const detail = await response.json();
...
async function loadU2PluginCallLogs() {
    const container = element("#u2-plugin-call-list");
    setRegionState(container, "loading");
    const result = element("#u2-plugin-call-result").value;
    const query = new URLSearchParams({ page: "1", page_size: "100" });
    if (result)
        query.set("result", result);
    try {
        const response = await authenticatedFetch(`/api/v1/logs/u2-plugin-calls?${query}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const body = await response.json();
        element("#u2-plugin-call-count").textContent =
            `共 ${body.total_count.toLocaleString("zh-CN")} 次调用`;
        if (body.items.length === 0) {
        ...
        renderRegionMessage(container, "error", `U2 插件调用日志读取失败：${errorMessage(error, "未知错误")}`);
    }
}
async function loadMikanPublishGroups() {
    const container = element("#mikan-publish-group-list");
    setRegionState(container, "loading");
    try {
...
async function saveMikanPublishGroup(item, name) {
...
async function refreshMikanPublishGroup(item) {
    const response = await authenticatedFetch(`/api/v1/mikan/publish-groups/${item.groupid}/refresh`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body: JSON.stringify({ expected_revision: item.revision }),
    });
...
async function clearTrustedOffset(item) {
    if (!window.confirm(`清理 Mikan ${item.mikanid} / Group ${item.groupid} 的自动证据与缓存？人工规则、完成记录和媒体文件不会删除。`))
        return;
    try {
        const response = await authenticatedFetch(`/api/v1/mikan/trusted-offsets/${item.mikanid}/${item.groupid}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
...
function updateManualMetadataSearchMode(clearIdentity = false) {
    const mediaType = element("#manual-metadata-media-type").value;
    element("#manual-metadata-query-label").textContent = mediaType === "tv"
        ? "TV 名称"
        : "Movie 名称";
    element("#manual-metadata-search").textContent = mediaType === "tv"
        ? "搜索 TMDB TV"
        : "搜索 TMDB Movie";
    element("#manual-metadata-search-results").replaceChildren();
    renderAnitomyTitleCandidates(element("#manual-metadata-anitomy-preview"), null, "", () => undefined);
    if (clearIdentity) {
        element("#manual-metadata-tmdb-id").value = "";
        element("#manual-metadata-season").value = "";
    }
}
function renderManualAnitomyTitle(result, message = "") {
    renderAnitomyTitleCandidates(element("#manual-metadata-anitomy-preview"), result, message, title => {
        const mediaType = element("#manual-metadata-media-type").value;
        const normalized = mediaType === "movie"
            ? normalizeMixedMediaMovieSearchTitle(title)
            : title.trim();
        element("#manual-metadata-title-query").value = normalized || title;
        element("#manual-metadata-search-results").replaceChildren();
    });
}
function manualMetadataTitleSource() {
    const preview = activeManualMetadataPreview;
    return preview?.title ?? "";
}
async function parseManualMetadataTitle() {
    const sourceText = manualMetadataTitleSource();
    if (!sourceText)
        return;
    const button = element("#manual-metadata-anitomy-parse");
    button.disabled = true;
    renderManualAnitomyTitle(null, "AnitomySharp 正在解析标题…");
    try {
        const response = await authenticatedFetch("/api/v1/metadata/anitomy/parse-title", {
            method: "POST",
            headers: jsonRequestHeaders(),
        ...
        if (result.success && result.anime_title) {
            ...
            element("#manual-metadata-title-query").value = normalized || result.anime_title;
            element("#manual-metadata-search-results").replaceChildren();
        }
    }
    catch (error) {
...
function manualMetadataSearchQueries(primary) {
    const preview = activeManualMetadataPreview;
    const mediaType = element("#manual-metadata-media-type").value;
    const normalize = (value) => mediaType === "movie"
    ...
    const append = (value) => {
        ...
        if (normalized.length < 2 || normalized.length > 256 || seen.has(key))
            return;
        seen.add(key);
    ...
    for (const source of [preview.title, ...preview.files.map(file => file.relative_path)]) {
        for (const match of source.matchAll(/\[([^\[\]]{2,256})\]/gu)) {
            ...
            if (isMixedMediaTechnicalTitle(candidate))
                continue;
            append(candidate);
            if (queries.length >= 8)
                return queries;
...
async function searchManualMetadataTitles() {
    const mediaType = element("#manual-metadata-media-type").value;
    const query = element("#manual-metadata-title-query").value.trim();
    const results = element("#manual-metadata-search-results");
    const message = element("#manual-metadata-assignment-message");
    if (!query) {
        message.textContent = "请先填写或解析作品名称。";
        return;
    }
    const searchButton = element("#manual-metadata-search");
    ...
    try {
        ...
        for (const candidate of queries) {
            ...
            const response = await authenticatedFetch(`/api/v1/tmdb/${endpoint}/search?query=${encodeURIComponent(candidate)}`, { headers });
            if (!response.ok)
                throw new Error(await responseError(response));
            if (mediaType === "tv") {
                const body = await response.json();
        ...
        for (const candidate of candidates) {
            ...
            select.addEventListener("click", () => {
                results.querySelectorAll(".manual-metadata-search-result")
                    .forEach(item => {
                ...
                marker.textContent = "●";
                element("#manual-metadata-tmdb-id").value = String(candidate.id);
...
async function openManualMetadataAssignment(taskId) {
    ...
    try {
        ...
        element("#manual-metadata-title-query").value = preview.title;
...
async function readaptOtherFiles(taskId, button) {
    ...
    try {
        const previewResponse = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(taskId)}/other-readaptation/preview`, { headers });
        ...
        button.textContent = "重新入队中…";
        const response = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(taskId)}/other-readaptation`, { method: "POST", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        await loadMetadataTasks();
    }
...
async function ignoreOtherAttention(taskId, button) {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(taskId)}/other-attention/ignore`, { method: "POST", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        await loadMetadataTasks();
...
function mixedMediaAssignments() {
    const assignments = Array.from(document.querySelectorAll('select[name="mixed-media-file-role"]'));
...
function showMixedMediaEditor() {
    mixedMediaReviewReady = false;
    element("#mixed-media-postprocess-editor").hidden = false;
    element("#mixed-media-postprocess-review").hidden = true;
    element("#mixed-media-postprocess-edit").hidden = true;
...
function recommendedMixedMediaMainId(preview) {
    const candidates = preview.files.filter(file => file.source_available
        && (preview.media_type === "movie"
            ? file.movie_role === "extras"
            : file.movie_hint && file.movie_role === null));
    const largest = candidates.reduce((current, file) => current === null || file.size_bytes > current.size_bytes ? file : current, null);
    return largest?.task_file_id ?? null;
}
function anitomyTitleCandidateRanges(result) {
    const ranges = [];
    for (const match of result.source_text.matchAll(/\[([^\[\]]{2,256})\]/gu)) {
        if (match.index === undefined)
            continue;
        const title = match[1].trim();
        if (isMixedMediaTechnicalTitle(title))
            continue;
    ...
    if (primaryValid && !ranges.some(range => result.match_start >= range.start
...
function renderAnitomyTitleCandidates(preview, result, message, selectTitle) {
    preview.replaceChildren();
    if (result === null) {
...
function renderAnitomyMovieTitle(result, message = "") {
    renderAnitomyTitleCandidates(element("#mixed-media-anitomy-preview"), result, message, title => {
        const normalized = normalizeMixedMediaMovieSearchTitle(title);
        element("#mixed-media-movie-query").value = normalized || title;
        selectedMixedMediaMovie = null;
...
async function parseMixedMediaMovieTitle() {
    ...
    const sourceText = preview.files.find(file => file.task_file_id === mainId)?.source_name
        ?? preview.title;
    const button = element("#mixed-media-anitomy-parse");
    button.disabled = true;
...
function renderMixedMediaReview() {
    const preview = activeMixedMediaPreview;
    const movie = selectedMixedMediaMovie;
    const assignments = mixedMediaAssignments();
    const review = element("#mixed-media-postprocess-review-files");
    review.replaceChildren();
    if (!preview || !movie || assignments.movieTaskFileId === null)
        return false;
    const roles = new Map([
        [assignments.movieTaskFileId, "movie"],
        ...assignments.movieExtraTaskFileIds.map(fileId => [fileId, "extras"]),
    ]);
    ...
    element("#mixed-media-postprocess-review-movie").textContent =
...
async function openMixedMediaPostprocess(taskId) {
    ...
    try {
        ...
        element("#mixed-media-postprocess-title").textContent = preview.media_type === "movie"
            ? "多 Movie 合集后处理"
            : "TV+Movie 后处理";
        element("#mixed-media-postprocess-purpose").textContent = preview.media_type === "movie"
...
async function searchMixedMediaMovies() {
    ...
    try {
        ...
        let matchedQuery = query;
        for (const candidate of queries) {
            const response = await authenticatedFetch(`/api/v1/tmdb/movies/search?query=${encodeURIComponent(candidate)}`, { headers });
            if (!response.ok)
                throw new Error(await responseError(response));
...
function mixedMediaMovieSearchQueries(primary) {
    const queries = [];
    const seen = new Set();
    const append = (value) => {
...
function isMixedMediaTechnicalTitle(value) {
...
function readaptationCell(value, code = false) {
    const cell = document.createElement("td");
    if (code) {
...
function appendReadaptationRow(body, label, before, after, code = false) {
    const row = document.createElement("tr");
...
function readaptationNumberInput(label, value) {
    const field = document.createElement("label");
...
function createReadaptationManualOverride(preview, file) {
    const form = document.createElement("form");
    ...
    const series = readaptationNumberInput("TMDB Series ID", file.after_tmdb_series_id ?? file.before_tmdb_series_id);
    const season = readaptationNumberInput("Season", file.after_tmdb_season_number ?? file.before_tmdb_season_number);
    const episode = readaptationNumberInput("Episode", file.after_tmdb_episode_number ?? file.before_tmdb_episode_number);
    ...
    form.addEventListener("submit", event => {
        ...
        void (async () => {
            try {
                const requestHeaders = new Headers(headers);
                requestHeaders.set("Content-Type", "application/json");
                const response = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(preview.task_id)}`
                ...
                const refreshedResponse = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(preview.task_id)}`
                    + "/other-readaptation/review", { headers });
                if (!refreshedResponse.ok)
                    throw new Error(await responseError(refreshedResponse));
...
function renderOtherReadaptationReview(preview) {
    const summary = element("#other-readaptation-review-summary");
    ...
    const duplicateCount = preview.files.filter(file => file.after_disposition === "duplicate"
...
async function approveOtherReadaptation(taskId, button) {
    ...
    try {
        const previewResponse = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(taskId)}/other-readaptation/review`, { headers });
        if (!previewResponse.ok)
            throw new Error(await responseError(previewResponse));
        const preview = await previewResponse.json();
...
async function confirmOtherReadaptationReview() {
...
async function loadMetadataDetail(taskId, target, button) {
    ...
    button.setAttribute("aria-expanded", "true");
    try {
        const response = await authenticatedFetch(`/api/v1/metadata/tasks/${encodeURIComponent(taskId)}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const detail = await response.json();
        ...
        if (body.items.length === 0) {
            ...
            target.replaceChildren(...body.items.map((attempt) => {
                ...
                const fileNames = linkedFileNames.length > 0
                    ? linkedFileNames
                    : taskFiles.map((file) => file.source_name).filter((name) => Boolean(name));
...
function openDownloadSummaryFromOverview(bucket) {
...
function openAllMetadataFromOverview() {
    metadataState.search = "";
    metadataState.status = "";
    metadataState.handling = "all";
    metadataState.file_state = "all";
...
function setOverviewCount(id, value) {
    element(`#${id}`).textContent = value === "!"
        ? value
        : value.toLocaleString("zh-CN");
}
function setOverviewFailure(ids, detailId) {
    for (const id of ids)
        setOverviewCount(id, "!");
    if (detailId)
        element(`#${detailId}`).textContent = "读取失败，点击进入详情";
}
...
async function loadOverviewStatistics() {
    ...
    if (pendingTmdb.status === "fulfilled") {
        ...
        const fallbackRecords = pendingTmdb.value.items.reduce((total, item) => total + item.fallback_record_count, 0);
        element("#overview-pending-tmdb-detail").textContent =
    ...
    if (sources.status === "fulfilled") {
        setOverviewCount("overview-sources-enabled-count", sources.value.items.filter(source => source.enabled).length);
        element("#overview-sources-detail").textContent =
...
let pendingMatchingLogTaskId = null;
...
function matchingLogEpisodeIdentity(item) {
    if (item.episode_numbers.length === 0)
        return null;
...
function matchingLogEpisodeLabel(item) {
    if (item.movie_file_count > 0 && item.episode_file_count === 0)
        return "不适用（电影）";
    return matchingLogEpisodeIdentity(item) ?? "尚未确认";
}
function openMetadataTaskFromMatchingLog(taskId) {
    metadataState.search = taskId;
    metadataState.page = 1;
    element("#metadata-search").value = taskId;
    saveMetadataState();
    selectWorkspace("tasks", "metadata");
}
async function loadMatchingLogs() {
    const list = element("#matching-log-list");
    setRegionState(list, "loading");
    const requestSequence = ++matchingLogRequestSequence;
    const search = element("#matching-log-search").value.trim();
    const query = new URLSearchParams({
        page: "1",
        page_size: "25",
        handling: "all",
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(await responseError(response));
        const body = await response.json();
        if (requestSequence !== matchingLogRequestSequence)
            return;
        element("#matching-log-status").textContent =
        ...
        if (autoExpandTaskId !== null) {
            requestAnimationFrame(() => {
                list.querySelector(`[data-task-id="${CSS.escape(autoExpandTaskId)}"]`)
        ...
        if (requestSequence !== matchingLogRequestSequence)
            return;
        element("#matching-log-status").textContent = "匹配日志读取失败";
    ...
    try {
        ...
        const cards = body.items.map((item) => {
            ...
            if (item.status === "organized" && item.readaptation_review_state === "approved") {
                ...
                reviewResult.addEventListener("click", () => void approveOtherReadaptation(item.task_id, reviewResult));
...
function createPendingRecoveryForm(bgmid, detail) {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        try {
            const requestHeaders = new Headers(headers);
            requestHeaders.set("Content-Type", "application/json");
            const response = await authenticatedFetch(`/api/v1/metadata/pending-tmdb/${encodeURIComponent(String(bgmid))}/recover`, {
                method: "POST",
                headers: requestHeaders,
...
async function loadPendingTmdbDetail(bgmid, target, button) {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/metadata/pending-tmdb/${encodeURIComponent(String(bgmid))}`, { headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const detail = await response.json();
        const sections = [];
...
function openDownloaderConfig(instance) {
    ...
    element("#downloader-config-path-label").textContent =
        isPt ? "长期做种下载目录" : "下载目录";
    element("#downloader-config-path-help").textContent = isPt
...
async function deleteDownloaderOverride() {
    ...
    const message = element("#downloader-config-message");
    try {
        const response = await authenticatedFetch(`/api/v1/downloaders/${encodeURIComponent(instance.id)}?expected_configuration_revision=${downloaderConfigurationRevision}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        await loadDownloaders();
...
function updateSourceWarning() {
    ...
    const linkType = element("#source-link-type");
    const linkTypeState = element("#source-link-type-state");
    if (strategy === "move")
        seeding.value = "0";
    ...
    element("#source-warning").textContent =
...
function populateSourceForm(profile, templateAdapter = "mikan") {
    ...
    element("#source-link-type").value = profile?.link_type ?? "hard";
...
async function previewSourceRoute() {
    ...
    try {
        const requestHeaders = new Headers(headers);
        requestHeaders.set("Content-Type", "application/json");
        const response = await authenticatedFetch(`/api/v1/sources/${encodeURIComponent(current.id)}/route-preview`, {
            method: "POST",
            headers: requestHeaders,
            body: JSON.stringify({
...
async function runSavedSourceRss() {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/sources/${encodeURIComponent(sourceId)}/rss/run`, { method: "POST", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        const body = await response.json();
        renderManualRssResponse(body, result);
...
async function deleteMikanWorkRule() {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/mikan/work-rules/${mikanId}?expected_revision=${rule.revision}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        activeMikanWorkRule = null;
        await loadMikanWorkRule();
...
async function saveSource(event) {
    ...
    const common = {
        ...
        link_type: element("#source-link-type").value,
    ...
    try {
        const requestHeaders = new Headers(headers);
        requestHeaders.set("Content-Type", "application/json");
        const response = await authenticatedFetch(current ? `/api/v1/sources/${encodeURIComponent(current.id)}` : "/api/v1/sources", {
...
async function deleteSource() {
    ...
    try {
        const response = await authenticatedFetch(`/api/v1/sources/${encodeURIComponent(current.id)}?expected_revision=${current.revision}`, { method: "DELETE", headers });
        if (!response.ok)
            throw new Error(await responseError(response));
        activeSourceId = null;
        await loadSources();
...
element("#movie-library-search-form").addEventListener("submit", (event) => {
    event.preventDefault();
    movieLibrarySearch = element("#movie-library-search").value.trim();
    movieLibraryPage = 1;
    void loadMovieLibrary();
});
element("#movie-library-search-clear").addEventListener("click", () => {
    element("#movie-library-search").value = "";
    movieLibrarySearch = "";
    movieLibraryPage = 1;
    void loadMovieLibrary();
});
const changeMovieLibraryOrdering = () => {
    movieLibrarySort = element("#movie-library-sort").value;
    movieLibraryDirection = element("#movie-library-direction").value;
    movieLibraryPageSize = Number(element("#movie-library-page-size").value);
    movieLibraryPage = 1;
    void loadMovieLibrary();
};
element("#movie-library-sort").addEventListener("change", changeMovieLibraryOrdering);
element("#movie-library-direction").addEventListener("change", changeMovieLibraryOrdering);
element("#movie-library-page-size").addEventListener("change", changeMovieLibraryOrdering);
element("#movie-library-previous").addEventListener("click", () => {
    if (movieLibraryPage <= 1)
        return;
    movieLibraryPage--;
    void loadMovieLibrary();
});
element("#movie-library-next").addEventListener("click", () => {
    movieLibraryPage++;
    void loadMovieLibrary();
});
element("#library-external-import").addEventListener("click", () => void importExternalMedia("all"));
...
element("#mikan-season-completion-binding").addEventListener("change", () => void discoverMikanSeasonCompletionGroups());
element("#mikan-season-completion-preview").addEventListener("click", () => void previewMikanSeasonCompletion());
...
element("#u2-plugin-call-reload").addEventListener("click", () => void loadU2PluginCallLogs());
element("#mikan-publish-group-reload").addEventListener("click", () => void loadMikanPublishGroups());
...
element("#u2-plugin-call-result").addEventListener("change", () => void loadU2PluginCallLogs());
...
element("#webui-restart").addEventListener("click", () => void restartRuntime());
...
element("#u2-web-api-reload").addEventListener("click", () => void loadU2WebApiCompatibility());
element("#u2-web-api-form").addEventListener("submit", (event) => void saveU2WebApiCompatibility(event));
...
element("#movie-file-dialog-close").addEventListener("click", () => movieFileDialog.close());
element("#movie-file-dialog-cancel").addEventListener("click", () => movieFileDialog.close());
...
element("#source-link-type").addEventListener("change", updateSourceWarning);
...
element("#anidb-cache-reload").addEventListener("click", () => void loadAnidbTitleCache());
element("#anidb-cache-refresh").addEventListener("click", () => void refreshAnidbTitleCache());
element("#anidb-cache-settings").addEventListener("submit", event => void saveAnidbCacheSettings(event));
element("#anidb-cache-search").addEventListener("submit", event => {
    event.preventDefault();
    anidbCachePage = 1;
    anidbCachePageSize = Number(element("#anidb-cache-page-size").value);
    void loadAnidbTitles();
});
element("#anidb-cache-clear").addEventListener("click", () => {
    element("#anidb-cache-search").reset();
    anidbCachePage = 1;
    anidbCachePageSize = 25;
    void loadAnidbTitles();
});
element("#anidb-cache-previous").addEventListener("click", () => {
    if (anidbCachePage <= 1)
        return;
    anidbCachePage--;
    void loadAnidbTitles();
});
element("#anidb-cache-next").addEventListener("click", () => {
    if (anidbCachePage * anidbCachePageSize >= anidbCacheTotalItems)
        return;
    anidbCachePage++;
    void loadAnidbTitles();
});
...
element("#other-readaptation-review-close").addEventListener("click", () => {
    if (!otherReadaptationReviewSubmitting)
        otherReadaptationReviewDialog.close();
});
...
element("#manual-metadata-media-type").addEventListener("change", () => {
    renderManualMetadataFiles();
    updateManualMetadataSearchMode(true);
    if (activeManualMetadataPreview) {
        element("#manual-metadata-title-query").value =
            activeManualMetadataPreview.title;
    }
});
element("#manual-metadata-anitomy-parse").addEventListener("click", () => void parseManualMetadataTitle());
element("#manual-metadata-search").addEventListener("click", () => void searchManualMetadataTitles());
...
element("#mixed-media-postprocess-edit").addEventListener("click", showMixedMediaEditor);
...
otherReadaptationReviewDialog.addEventListener("close", () => {
    ...
    element("#other-readaptation-review-summary").replaceChildren();
    element("#other-readaptation-review-files").replaceChildren();
    element("#other-readaptation-review-message").textContent = "";
    otherReadaptationReviewConfirm.disabled = false;
...
element("#live-log-exception-only").addEventListener("change", renderLiveLogs);
document.querySelectorAll("[data-log-domain]").forEach(button => {
    button.addEventListener("click", () => {
...
element("#ai-log-filters").addEventListener("submit", event => {
    event.preventDefault();
    aiLogPage = 1;
    aiLogPageSize = Number(element("#ai-log-page-size").value);
    void loadAiInvocationLogs();
});
element("#ai-log-reset").addEventListener("click", () => {
    element("#ai-log-filters").reset();
    element("#ai-log-page-size").value = "25";
    aiLogPage = 1;
    aiLogPageSize = 25;
    void loadAiInvocationLogs();
});
element("#ai-log-reload").addEventListener("click", () => void loadAiInvocationLogs());
element("#ai-log-previous").addEventListener("click", () => {
    if (aiLogPage <= 1)
        return;
    aiLogPage--;
    void loadAiInvocationLogs();
});
element("#ai-log-next").addEventListener("click", () => {
    if (aiLogPage * aiLogPageSize >= aiLogTotalItems)
        return;
    aiLogPage++;
    void loadAiInvocationLogs();
});
element("#matching-log-filters").addEventListener("submit", event => {
    event.preventDefault();
    pendingMatchingLogTaskId = null;
    void loadMatchingLogs();
});
element("#matching-log-reset").addEventListener("click", () => {
    element("#matching-log-search").value = "";
    pendingMatchingLogTaskId = null;
    void loadMatchingLogs();
});
element("#matching-log-reload").addEventListener("click", () => void loadMatchingLogs());
element("#ai-debug-close").addEventListener("click", () => aiDebugDialog.close());
element("#ai-debug-delete").addEventListener("click", () => void deleteActiveAiDebugChain());
...
element("#webui-access-key-open").addEventListener("click", () => {
    void requestWebUiAccessKey(webUiAccessKey
        ? "输入新的 web.webui_access_key；验证成功后立即替换当前浏览器凭据。"
        : "输入 web.webui_access_key；验证成功后会自动加载页面数据。");
});
element("#webui-access-key-cancel").addEventListener("click", () => {
    finishWebUiAccessKeyRequest(null);
});
element("#webui-access-key-clear").addEventListener("click", () => {
