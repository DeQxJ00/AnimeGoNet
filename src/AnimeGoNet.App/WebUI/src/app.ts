...
let activeMikanSeasonCompletion: MikanSeasonCompletionPreview | null = null;
...
function selectedMikanSeasonCompletionBinding(): AnimeSeasonMikanBinding | null {
...
function renderMikanSeasonCompletion(preview: MikanSeasonCompletionPreview): void {
...
async function confirmMikanSeasonCompletion(): Promise<void> {
  ...
  const selected = Array.from(document.querySelectorAll<HTMLInputElement>(
    "#mikan-season-completion-items input[data-mikan-completion-candidate]:checked",
  )).map((checkbox) => checkbox.dataset.mikanCompletionCandidate ?? "").filter(Boolean);
  ...
  try {
    ...
    if (!response.ok) throw new Error(await responseError(response));
    const result = await response.json() as ManualRssResponse;
    const staged = result.items.filter((item) => item.ingest_task_id !== null).length;
    const alreadyKnown = result.items.filter((item) =>
