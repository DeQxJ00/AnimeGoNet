...
interface MetadataItem {
  ...
  movie_run_id: string | null;
  movie_attempt_id: string | null;
...
interface MixedMediaPostprocessPreview {
  task_id: string;
  title: string;
  media_type: "tv" | "movie";
  eligible: boolean;
  ...
  files: Array<{
    ...
    movie_hint: boolean;
    pending_postprocess: boolean;
    source_available: boolean;
  }>;
}

interface ManualMetadataAssignmentPreview {
  task_id: string;
  title: string;
  status: string;
  media_type: "tv" | "movie";
...
interface TmdbSeriesSearchItem {
  tmdb_series_id: number;
  name: string;
...
interface AnimeSeasonListItem {
  ...
  last_episode_changed_at_utc: string | null;
...
interface AnimeSeasonMikanBinding {
  source_profile_id: string;
  mikanid: number;
  groupid: number | null;
...
interface AnimeSeasonDetail {
  ...
  mikan_bindings: AnimeSeasonMikanBinding[];
  related_task_total: number;
...
interface MikanSeasonCompletionCandidate {
  candidate_id: string;
  title: string;
  length: number;
...
interface MikanSeasonCompletionPreview {
  tmdb_series_id: number;
  tmdb_season_number: number;
  resource_revision: string;
  source_profile_id: string;
  ...
  items: MikanSeasonCompletionCandidate[];
}

interface MikanSeasonCompletionGroup {
  groupid: number;
  name: string;
  previously_used: boolean;
}

interface MikanSeasonCompletionGroupDiscovery {
  source_profile_id: string;
  mikanid: number;
  groups: MikanSeasonCompletionGroup[];
}

interface MikanSeasonCompletionWork {
  source_profile_id: string;
  mikanid: number;
  last_used_at_utc: string;
  known_groupids: number[];
  discovery: MikanSeasonCompletionGroupDiscovery | null;
}

interface NamedMikanSeasonCompletionPreview extends MikanSeasonCompletionPreview {
  group_name: string;
}

interface MikanSeasonCompletionState {
  work: MikanSeasonCompletionWork;
  previews: NamedMikanSeasonCompletionPreview[];
  failures: string[];
}
...
interface MikanManualSeriesMappingItem {
  mikanid: number;
  groupid: number;
  expected_tmdb_series_id: number;
  tmdb_series_id: number;
  tmdb_season_number: number;
  accepted_from_task_id: string;
  accepted_at_utc: string;
  updated_at_utc: string;
}

interface MikanPluginCallLogItem {
  index: number;
  title: string | null;
  task_id: string | null;
  mikanid: number | null;
...
interface MikanPluginCallLog {
  id: string;
  endpoint: string;
  mode: "single" | "all" | "selected" | "batch";
  media_type: "tv" | "movie" | "mixed";
  ...
  started_at_utc: string;
  completed_at_utc: string;
  items: MikanPluginCallLogItem[];
}

interface MikanPluginCallLogList {
  page: number;
  page_size: number;
  total_count: number;
...
interface MikanTrustedOffsetBlacklistItem {
  scope: MikanTrustedOffsetBlacklistScope;
  mikanid: number | null;
  groupid: number | null;
  created_at_utc: string;
}
...
interface DeleteExecutionResponse {
  execution_id: string;
  task_id: string;
  ...
  items: Array<DeleteTarget & {
    kind: string;
    state: "pending" | "failed" | "completed" | "skipped";
  }>;
}
...
function jsonRequestHeaders(): Headers {
...
const mikanSeasonCompletionDialog = element<HTMLDialogElement>("#mikan-season-completion-dialog");
...
const mixedMediaPostprocessDialog = element<HTMLDialogElement>("#mixed-media-postprocess-dialog");
const mixedMediaPostprocessConfirm = element<HTMLButtonElement>("#mixed-media-postprocess-confirm");
const manualMetadataDialog = element<HTMLDialogElement>("#manual-metadata-assignment-dialog");
const manualMetadataConfirm = element<HTMLButtonElement>("#manual-metadata-assignment-confirm");
let activeManualMetadataPreview: ManualMetadataAssignmentPreview | null = null;
...
type EditableConfigurationSection = "paths" | "network" | "ai";
let activeConfigurationSection: EditableConfigurationSection = "paths";
...
let activeMikanSeasonCompletion: MikanSeasonCompletionState | null = null;
let activeMikanSeasonWorks: MikanSeasonCompletionWork[] = [];
...
function workspaceLeafTabs(definition: WorkspaceDefinition): WorkspaceTab[] {
...
function selectWorkspace(
  ...
  tabs.replaceChildren(...definition.tabs.map(tab => {
    if (tab.children?.length) {
      ...
      toggle.addEventListener("click", () => {
        if (!groupActive) selectWorkspace(workspace, tab.children![0].id);
      });
      ...
      submenu.replaceChildren(...tab.children.map(child => {
        ...
        if (child.id === selectedSubview) childButton.setAttribute("aria-current", "page");
  ...
  if (workspace === "tasks" && selectedSubview === "ai-invocations") void loadAiInvocationLogs();
  if (workspace === "tasks" && selectedSubview === "matching") void loadMatchingLogs();
...
function initializeWorkspaceNavigation(): void {
  ...
  window.addEventListener("hashchange", () => {
    ...
    selectWorkspace(
      target.workspace,
      target.subview,
      window.location.hash.startsWith("#/logs/"),
    );
  });
...
function metadataCardStatusText(value: string): string {
...
function bangumiArchiveUsageMessage(text: string, alert = false): HTMLTableRowElement {
  ...
  if (page.items.length === 0) {
    ...
    target.replaceChildren(...page.items.map(item => {
      ...
      for (const [index, value] of values.entries()) {
        ...
        if (index === 0) cell.className = "data-update-usage-kind";
        if (index === 1 || index === 2 || index === 5) cell.classList.add("data-update-usage-number");
        row.append(cell);
      }
      return row;
    }));
...
function liveLogWebSocketUrl(): string {
  ...
  url.search = "";
  if (webUiAccessKey) url.searchParams.set("webui_access_key", webUiAccessKey);
  return url.toString();
}
  ...
  if (result.items.length === 0) {
    ...
    const entries = result.items.map(item => {
      ...
      details.append(
        ...
        liveLogDetail("AI 触发原因", item.ai_trigger_reason ?? "历史调用未记录"),
...
function renderMovieLibraryPage(page: AnimeMovieListPage): void {
  ...
  renderRegionContent(list, ...page.items.map((item) => {
    ...
    const deleteMenu = document.createElement("span");
    deleteMenu.className = "movie-library-delete-menu";
    const deleteMenuLabel = document.createElement("button");
    deleteMenuLabel.type = "button";
    deleteMenuLabel.className = "delete-button";
    deleteMenuLabel.textContent = "删除";
    deleteMenuLabel.title = "打开电影删除选项";
    deleteMenuLabel.setAttribute("aria-haspopup", "menu");
    deleteMenuLabel.setAttribute("aria-expanded", "false");
    const deleteOptions = document.createElement("span");
    deleteOptions.className = "movie-library-delete-options";
    deleteOptions.id = `movie-library-delete-options-${item.tmdb_movie_id}`;
    deleteOptions.setAttribute("popover", "auto");
    deleteOptions.setAttribute("role", "menu");
    deleteMenuLabel.setAttribute("popovertarget", deleteOptions.id);
    deleteMenuLabel.setAttribute("aria-controls", deleteOptions.id);
    deleteMenuLabel.addEventListener("click", () => {
      requestAnimationFrame(() => {
        if (!deleteOptions.matches(":popover-open")) return;
        const trigger = deleteMenuLabel.getBoundingClientRect();
        const gap = 6;
        const margin = 8;
        const width = deleteOptions.offsetWidth;
        const height = deleteOptions.offsetHeight;
        const left = Math.min(
        ...
        deleteOptions.style.left = `${left}px`;
        deleteOptions.style.top = `${top}px`;
      });
    });
    deleteOptions.addEventListener("toggle", () => {
      deleteMenuLabel.setAttribute(
        "aria-expanded",
        String(deleteOptions.matches(":popover-open")),
      );
    });
    const deleteContent = document.createElement("button");
    deleteContent.type = "button";
    deleteContent.className = "danger-button";
    deleteContent.setAttribute("role", "menuitem");
    ...
    deleteContent.addEventListener("click", () => {
      deleteOptions.hidePopover();
      if (item.related_task_id) void openDeletePreview(item.related_task_id);
    });
    const remove = document.createElement("button");
    remove.type = "button";
    ...
    remove.addEventListener("click", () => {
      deleteOptions.hidePopover();
      void deleteMovieLibraryItem(item, remove);
    });
    deleteOptions.append(deleteContent, remove);
    deleteMenu.append(deleteMenuLabel, deleteOptions);
    actions.append(tmdbLink, files, deleteMenu);
...
async function openMovieFileList(item: AnimeMovieListItem): Promise<void> {
  ...
  const save = element<HTMLButtonElement>("#movie-file-save");
  ...
  try {
    ...
    if (detail.files.length === 0) {
      ...
      const editableVideos = detail.files.filter(file =>
        file.is_video
        && file.exists
      ...
      detail.files.forEach(file => {
        ...
        size.textContent = file.exists
          ...
          : "—";
        if (file.link_type === "symbolic") size.title = "符号链接目标文件的实际大小";
      ...
      save.disabled = true;
      save.onclick = async () => {
        if (!selectedMain || selectedMain === currentMain) return;
        save.disabled = true;
        forceDelete.disabled = true;
        message.textContent = "正在安全交换 Movie 主文件与 Extras，并同步任务引用…";
        try {
          const response = await authenticatedFetch(
            `/api/v1/library/movies/${item.tmdb_movie_id}/files/main`,
            {
              method: "PUT",
          ...
          const result = await response.json() as {
            ...
            former_main_relative_path: string | null;
          };
...
async function deleteMovieLibraryItem(
  item: AnimeMovieListItem,
  button: HTMLButtonElement,
): Promise<void> {
  if (!window.confirm(
    `仅删除 ${item.title}（TMDB Movie ${item.tmdb_movie_id}）的本地投影？`
    + " 服务端会拒绝仍有关联任务、完成记录或有效 claim 的电影。"
    + " 此操作不会删除下载器任务、下载源文件或媒体文件。",
  )) return;

  const status = element<HTMLElement>("#movie-library-action-status");
  button.disabled = true;
  status.textContent = `正在检查 ${item.title} 的引用并删除投影…`;
  try {
    const query = new URLSearchParams({ expected_revision: item.resource_revision });
    const response = await authenticatedFetch(
      `/api/v1/library/movies/${item.tmdb_movie_id}?${query}`,
      { method: "DELETE", headers },
    );
    if (!response.ok) throw new Error(await responseError(response));
...
function renderLibraryDetail(detail: AnimeSeasonDetail, focus: boolean): void {
  ...
  const tmdbLink = element<HTMLAnchorElement>("#library-detail-tmdb-link");
  tmdbLink.href = `https://www.themoviedb.org/tv/${detail.tmdb_series_id}/season/${detail.tmdb_season_number}`;
  tmdbLink.hidden = false;
  tmdbLink.setAttribute(
    "aria-label",
    `在 TMDB 打开 ${detail.display_name} ${detail.season_name}`,
  );
  const completionButton = element<HTMLButtonElement>("#library-detail-mikan-completion");
  completionButton.disabled = detail.mikan_bindings.length === 0;
  completionButton.title = detail.mikan_bindings.length === 0
    ? "该季度尚无带 mikanid 的关联任务"
  ...
  const manualAssignmentButton = element<HTMLButtonElement>("#library-detail-manual-assignment");
  ...
  element<HTMLElement>("#library-detail-action-status").textContent =
    detail.mikan_bindings.length === 0
      ? "尚无可用于 EP 自动补完的 mikanid；先通过 Mikan 导入至少一个关联任务。"
      : `可从 ${detail.mikan_bindings.length} 条已知 Mikan 作品关联发现字幕组；已有 groupid 会默认勾选。`;
...
async function loadLibraryDetail(
  ...
  element<HTMLButtonElement>("#library-detail-manual-assignment").disabled = true;
...
const mikanSeasonCompletionStatusLabels: Record<string, string> = {
...
function selectedMikanSeasonCompletionWork(): MikanSeasonCompletionWork | null {
  const index = Number(element<HTMLSelectElement>("#mikan-season-completion-binding").value);
  return Number.isInteger(index) ? activeMikanSeasonWorks[index] ?? null : null;
}

function selectedMikanSeasonCompletionGroups(): number[] {
  return Array.from(document.querySelectorAll<HTMLInputElement>(
    "#mikan-season-completion-groups input[data-mikan-completion-group]:checked",
  ))
    .map((checkbox) => Number(checkbox.dataset.mikanCompletionGroup))
    .filter((groupId) => Number.isInteger(groupId) && groupId > 0);
}

function updateMikanSeasonCompletionSelectionStatus(): void {
  const completion = activeMikanSeasonCompletion;
  const selected = document.querySelectorAll<HTMLInputElement>(
    "#mikan-season-completion-items input[data-mikan-completion-candidate]:checked",
  ).length;
  const confirm = element<HTMLButtonElement>("#mikan-season-completion-confirm");
  confirm.disabled = completion === null || selected === 0;
  if (!completion) return;
  const offsets = completion.previews.map((preview) => preview.episode_offset === null
    ? `${preview.group_name}: 无 Offset`
    : `${preview.group_name}: ${preview.episode_offset >= 0 ? "+" : ""}${preview.episode_offset}`);
  const failures = completion.failures.length === 0
    ? ""
    : ` · ${completion.failures.length} 个字幕组预览失败`;
  element<HTMLElement>("#mikan-season-completion-status").textContent =
    `字幕组 ${completion.previews.length} 个 · RSS ${
      completion.previews.reduce((sum, preview) => sum + preview.items.length, 0)
    } 条 · 已选 ${selected} 条 · ${offsets.join("；")}${failures}`;
}

function renderMikanSeasonCompletion(completion: MikanSeasonCompletionState): void {
  activeMikanSeasonCompletion = completion;
  const body = element<HTMLTableSectionElement>("#mikan-season-completion-items");
  const rows = completion.previews.flatMap((preview) => preview.items.map((item) => {
    const row = document.createElement("tr");
    if (item.status.startsWith("completed_")) row.className = "completed";
    const selectionCell = document.createElement("td");
    const checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.checked = item.default_selected;
    checkbox.dataset.mikanCompletionCandidate = item.candidate_id;
    checkbox.dataset.mikanCompletionGroup = String(preview.groupid);
    ...
    group.textContent = `${preview.group_name} · ${preview.groupid}`;
    ...
    if (item.status.startsWith("completed_")) badge.classList.add("completed");
    if (item.status === "missing_target_episode") badge.classList.add("missing");
    badge.textContent = mikanSeasonCompletionStatusLabels[item.status] ?? item.status;
    status.append(badge);
    row.append(selectionCell, group, sourceEpisode, targetEpisode, title, metadata, status);
    return row;
  }));
  if (rows.length === 0) {
    const row = document.createElement("tr");
...
function renderMikanSeasonCompletionGroups(
  discovery: MikanSeasonCompletionGroupDiscovery,
): void {
  const groups = element<HTMLElement>("#mikan-season-completion-groups");
  groups.replaceChildren(...discovery.groups.map((group) => {
    const label = document.createElement("label");
    if (group.previously_used) label.classList.add("previously-used");
    const checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.checked = group.previously_used;
    checkbox.dataset.mikanCompletionGroup = String(group.groupid);
    checkbox.addEventListener("change", () => {
      activeMikanSeasonCompletion = null;
      element<HTMLTableSectionElement>("#mikan-season-completion-items").replaceChildren();
      element<HTMLButtonElement>("#mikan-season-completion-confirm").disabled = true;
      const count = selectedMikanSeasonCompletionGroups().length;
      element<HTMLButtonElement>("#mikan-season-completion-preview").disabled = count === 0;
      element<HTMLElement>("#mikan-season-completion-status").textContent = count === 0
        ? "请选择至少一个字幕组。"
        : `已选 ${count} 个字幕组；点击“预览已选字幕组”读取季度 RSS。`;
    });
    const text = document.createElement("span");
    text.textContent = `${group.name} · groupid ${group.groupid}${
      group.previously_used ? " · 已用" : ""
    }`;
    label.append(checkbox, text);
    return label;
  }));
  element<HTMLButtonElement>("#mikan-season-completion-preview").disabled =
    selectedMikanSeasonCompletionGroups().length === 0;
}

async function discoverMikanSeasonCompletionGroups(): Promise<void> {
  const detail = activeLibraryDetail;
  const work = selectedMikanSeasonCompletionWork();
  if (!detail || !work) return;
  const sequence = ++mikanSeasonCompletionRequestSequence;
  activeMikanSeasonCompletion = null;
  element<HTMLButtonElement>("#mikan-season-completion-preview").disabled = true;
  element<HTMLButtonElement>("#mikan-season-completion-confirm").disabled = true;
  element<HTMLTableSectionElement>("#mikan-season-completion-items").replaceChildren();
  const groups = element<HTMLElement>("#mikan-season-completion-groups");
  groups.textContent = `正在从 mikanid ${work.mikanid} 的作品页读取字幕组…`;
  element<HTMLElement>("#mikan-season-completion-status").textContent =
    "正在发现字幕组；不会创建下载任务。";
  try {
    const requestHeaders = new Headers(headers);
    requestHeaders.set("Content-Type", "application/json");
    const response = await authenticatedFetch(
      `/api/v1/library/seasons/${detail.tmdb_series_id}/${detail.tmdb_season_number}/mikan-completion/groups`,
      {
        method: "POST",
        headers: requestHeaders,
        body: JSON.stringify({
          source_profile_id: work.source_profile_id,
          mikanid: work.mikanid,
        }),
      },
    );
    if (!response.ok) throw new Error(await responseError(response));
    const discovery = await response.json() as MikanSeasonCompletionGroupDiscovery;
    if (sequence !== mikanSeasonCompletionRequestSequence || !mikanSeasonCompletionDialog.open) return;
    work.discovery = discovery;
    renderMikanSeasonCompletionGroups(discovery);
    const selected = selectedMikanSeasonCompletionGroups().length;
    element<HTMLElement>("#mikan-season-completion-status").textContent = selected > 0
      ? `发现 ${discovery.groups.length} 个字幕组，已默认选择 ${selected} 个历史 groupid；正在读取对应 RSS…`
      : `发现 ${discovery.groups.length} 个字幕组；请选择后预览 RSS。`;
    if (selected > 0) await previewMikanSeasonCompletion();
  } catch (error) {
    if (sequence !== mikanSeasonCompletionRequestSequence) return;
    groups.textContent = "字幕组列表读取失败。";
    element<HTMLElement>("#mikan-season-completion-status").textContent =
      `字幕组发现失败：${errorMessage(error, "未知错误")}`;
  }
}

async function previewMikanSeasonCompletion(): Promise<void> {
  const detail = activeLibraryDetail;
  const work = selectedMikanSeasonCompletionWork();
  const groupIds = selectedMikanSeasonCompletionGroups();
  if (!detail || !work || !work.discovery || groupIds.length === 0) return;
  const sequence = ++mikanSeasonCompletionRequestSequence;
  activeMikanSeasonCompletion = null;
  element<HTMLButtonElement>("#mikan-season-completion-confirm").disabled = true;
  element<HTMLTableSectionElement>("#mikan-season-completion-items").replaceChildren();
  element<HTMLElement>("#mikan-season-completion-status").textContent =
    `正在读取 mikanid ${work.mikanid} 的 ${groupIds.length} 个字幕组季度 RSS…`;
  try {
    const requestHeaders = new Headers(headers);
    requestHeaders.set("Content-Type", "application/json");
    const settled = await Promise.allSettled(groupIds.map(async (groupId) => {
      const response = await authenticatedFetch(
        `/api/v1/library/seasons/${detail.tmdb_series_id}/${detail.tmdb_season_number}/mikan-completion/preview`,
        {
          method: "POST",
          headers: requestHeaders,
          body: JSON.stringify({
      ...
      if (!response.ok) throw new Error(await responseError(response));
      const preview = await response.json() as MikanSeasonCompletionPreview;
      const group = work.discovery?.groups.find((item) => item.groupid === groupId);
      return { ...preview, group_name: group?.name ?? `groupid ${groupId}` };
    }));
    if (sequence !== mikanSeasonCompletionRequestSequence || !mikanSeasonCompletionDialog.open) return;
    ...
    const failures = settled.flatMap((result) => result.status === "rejected"
      ? [errorMessage(result.reason, "未知错误")]
      : []);
    if (previews.length === 0) throw new Error(failures.join("；") || "所有字幕组 RSS 预览均失败");
    renderMikanSeasonCompletion({ work, previews, failures });
  } catch (error) {
    if (sequence !== mikanSeasonCompletionRequestSequence) return;
    element<HTMLElement>("#mikan-season-completion-status").textContent =
...
function openMikanSeasonCompletion(): void {
  const detail = activeLibraryDetail;
  if (!detail) return;
  if (detail.mikan_bindings.length === 0) {
    element<HTMLElement>("#library-detail-action-status").textContent =
      "无法自动补完：该季度尚无包含 mikanid 的关联任务。";
    return;
  }
  activeMikanSeasonCompletion = null;
  const works = new Map<string, MikanSeasonCompletionWork>();
  detail.mikan_bindings.forEach((binding) => {
    const key = `${binding.source_profile_id}\u0000${binding.mikanid}`;
    const existing = works.get(key);
    if (!existing) {
      works.set(key, {
        source_profile_id: binding.source_profile_id,
        mikanid: binding.mikanid,
        last_used_at_utc: binding.last_used_at_utc,
        known_groupids: binding.groupid === null ? [] : [binding.groupid],
        discovery: null,
      });
    } else if (binding.groupid !== null && !existing.known_groupids.includes(binding.groupid)) {
      existing.known_groupids.push(binding.groupid);
    }
  });
  activeMikanSeasonWorks = Array.from(works.values());
  const bindings = element<HTMLSelectElement>("#mikan-season-completion-binding");
  bindings.replaceChildren(...activeMikanSeasonWorks.map((work, index) => {
    const option = document.createElement("option");
    option.value = String(index);
    const known = work.known_groupids.length === 0
      ? "groupid 待发现"
      : `已有 groupid ${work.known_groupids.join(", ")}`;
    option.textContent = `${work.source_profile_id} · mikanid ${work.mikanid} · ${known} · 最近 ${
      libraryDate(work.last_used_at_utc, true)
    }`;
    return option;
  }));
  element<HTMLElement>("#mikan-season-completion-title").textContent =
    `${detail.display_name} · ${detail.season_name} · EP 自动补完`;
  element<HTMLTableSectionElement>("#mikan-season-completion-items").replaceChildren();
  element<HTMLElement>("#mikan-season-completion-groups").replaceChildren();
  if (!mikanSeasonCompletionDialog.open) mikanSeasonCompletionDialog.showModal();
  void discoverMikanSeasonCompletionGroups();
}

function setMikanSeasonCompletionSelection(mode: "default" | "all" | "none"): void {
  document.querySelectorAll<HTMLInputElement>(
    "#mikan-season-completion-items input[data-mikan-completion-candidate]",
  ).forEach((checkbox) => {
...
function closeMikanSeasonCompletion(): void {
  mikanSeasonCompletionRequestSequence++;
  activeMikanSeasonCompletion = null;
  activeMikanSeasonWorks = [];
  if (mikanSeasonCompletionDialog.open) mikanSeasonCompletionDialog.close();
}

async function confirmMikanSeasonCompletion(): Promise<void> {
  const detail = activeLibraryDetail;
  const completion = activeMikanSeasonCompletion;
  if (!detail || !completion) return;
  const selected = Array.from(document.querySelectorAll<HTMLInputElement>(
    "#mikan-season-completion-items input[data-mikan-completion-candidate]:checked",
  ));
  ...
  if (selected.length > 12 && !window.confirm(
    `本次将提交 ${selected.length} 条 Mikan 候选，超过 12 集。确认继续？`,
  )) return;
  ...
  try {
    const requestHeaders = new Headers(headers);
    requestHeaders.set("Content-Type", "application/json");
    const selectedByGroup = new Map<number, string[]>();
    selected.forEach((checkbox) => {
      const groupId = Number(checkbox.dataset.mikanCompletionGroup);
      const candidateId = checkbox.dataset.mikanCompletionCandidate ?? "";
      const values = selectedByGroup.get(groupId) ?? [];
      values.push(candidateId);
      selectedByGroup.set(groupId, values);
    });
    const settled = await Promise.allSettled(Array.from(selectedByGroup.entries()).map(
      async ([groupId, candidateIds]) => {
        const preview = completion.previews.find((item) => item.groupid === groupId);
        if (!preview) throw new Error(`groupid ${groupId} 缺少有效预览`);
        const response = await authenticatedFetch(
          `/api/v1/library/seasons/${detail.tmdb_series_id}/${detail.tmdb_season_number}/mikan-completion`,
          {
            method: "POST",
            headers: requestHeaders,
            body: JSON.stringify({
        ...
        if (!response.ok) throw new Error(await responseError(response));
        return await response.json() as ManualRssResponse;
      },
    ));
    ...
    const staged = items.filter((item) => item.ingest_task_id !== null).length;
    const alreadyKnown = items.filter((item) =>
      ["already_ingested", "already_completed"].includes(item.status)).length;
    const rejected = items.length - staged - alreadyKnown;
    const summary = `Mikan 补完 ${results.length} 个字幕组：提交任务 ${staged}，已存在 ${alreadyKnown}，被规则拒绝/失败 ${rejected}`;
    if (failures.length === 0) {
      closeMikanSeasonCompletion();
      element<HTMLElement>("#library-detail-action-status").textContent = `${summary}。`;
    } else {
      element<HTMLElement>("#mikan-season-completion-status").textContent =
        `${summary}；另有 ${failures.length} 个字幕组提交失败：${failures.join("；")}`;
...
function renderSubtitleImportCandidates(): void {
  ...
  const subtitleRenameSuffix = (fileName: string): string => {
    const languageSuffix = fileName.match(
      /\.[A-Za-z]{2,8}(?:[-_][A-Za-z0-9]{2,8})?\.(?:ass|ssa|srt|vtt|sub|idx|sup|smi|ttml|xml)$/i,
    );
    if (languageSuffix) return languageSuffix[0];
    const extension = fileName.match(/\.[^.]+$/);
    return extension?.[0] ?? "";
  };
  const targetName = (
    candidate: SubtitleArchiveCandidate,
    enabled: HTMLInputElement,
    episode: HTMLInputElement,
  ): string => {
    if (!enabled.checked) return "不导入";
  ...
  if (firstCandidate && firstRow) {
    const enabled = firstRow.querySelector<HTMLInputElement>("[data-subtitle-enabled]")!;
    const episode = firstRow.querySelector<HTMLInputElement>("[data-subtitle-episode]")!;
...
function openLibraryManualAssignment(): void {
  if (!activeLibraryDetail) return;
  const detail = activeLibraryDetail;
  const status = element<HTMLElement>("#library-detail-action-status");
  if (detail.related_tasks.length === 0) {
  ...
  group.open = true;
  group.scrollIntoView({ behavior: "smooth", block: "center" });
  group.querySelector<HTMLButtonElement>("button[data-library-manual-task]")
...
async function loadConfiguration(): Promise<void> {
  ...
  const networkContainer = element<HTMLElement>("#configuration-network");
  const aiContainer = element<HTMLElement>("#configuration-ai");
  const networkStatus = element<HTMLElement>("#configuration-network-status");
  const aiStatus = element<HTMLElement>("#configuration-ai-status");
  ...
  try {
    ...
    if (config.migration_diagnostics.length > 0) {
      pathCards.unshift(configurationCard(
        "旧配置迁移阻断",
...
async function loadWebUiAuthentication(): Promise<void> {
  ...
  const portInput = element<HTMLInputElement>("#webui-listen-port");
  const keyInput = element<HTMLInputElement>("#webui-authentication-access-key");
  const reload = element<HTMLButtonElement>("#webui-authentication-reload");
  const save = element<HTMLButtonElement>("#webui-authentication-save");
  ...
  try {
    ...
    if (!runtimeResponse.ok) throw new Error(await responseError(runtimeResponse));
    const runtime = await runtimeResponse.json() as RuntimeConfiguration;
...
async function saveWebUiAuthentication(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  const status = element<HTMLElement>("#webui-authentication-status");
  const hostInput = element<HTMLInputElement>("#webui-listen-host");
  const portInput = element<HTMLInputElement>("#webui-listen-port");
...
function deploymentPaths(configuration: JsonObject): JsonObject {
...
function isAbsoluteConfigurationPath(value: string): boolean {
  return value.startsWith("/")
    || /^[a-z]:[\\/]/iu.test(value)
    || /^\\\\[^\\]+\\[^\\]+/u.test(value);
}

async function loadDeploymentDataPath(): Promise<void> {
  const input = element<HTMLInputElement>("#deployment-data-path");
  const status = element<HTMLElement>("#deployment-data-path-status");
  const reload = element<HTMLButtonElement>("#deployment-data-path-reload");
  const save = element<HTMLButtonElement>("#deployment-data-path-save");
  reload.disabled = true;
  save.disabled = true;
...
async function saveDeploymentDataPath(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  const input = element<HTMLInputElement>("#deployment-data-path");
  const status = element<HTMLElement>("#deployment-data-path-status");
  const reload = element<HTMLButtonElement>("#deployment-data-path-reload");
  ...
  try {
    ...
    if (!response.ok) throw new Error(await responseError(response));
    const envelope = await response.json() as LegacyApiEnvelope<unknown>;
    if (envelope.code !== 200) throw new Error(envelope.msg || `配置接口返回 code ${envelope.code}`);
...
async function loadU2WebApiCompatibility(): Promise<void> {
  ...
  element<HTMLInputElement>("#u2-web-api-url").value = animeGoHelperApiUrl();
...
const configurationSectionSelectors: Record<EditableConfigurationSection, string[]> = {
  paths: [
...
function configureConfigurationEditorSection(section: EditableConfigurationSection): void {
  ...
  for (const selector of configurationSectionSelectors[section]) {
    let node = element<HTMLElement>(selector);
    while (node.parentElement !== grid) {
      const parent = node.parentElement;
      if (parent === null) throw new Error(`配置字段不在编辑网格内：${selector}`);
      node = parent;
    }
    visible.add(node);
  }
  for (const child of Array.from(grid.children) as HTMLElement[]) {
    ...
    if (child.hidden) {
      child.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | HTMLButtonElement>(
        "input, select, textarea, button",
      ).forEach(control => { control.disabled = true; });
    }
  }
  const titles: Record<EditableConfigurationSection, string> = {
    paths: "编辑目录与路径",
    network: "编辑网络与代理",
    ai: "编辑 AI 与 MCP",
  };
...
function openConfigurationEditor(section: EditableConfigurationSection): void {
  ...
  element<HTMLElement>("#configuration-dialog .configuration-form-grid")
...
async function confirmConfiguration(): Promise<void> {
  ...
  try {
    ...
    requestHeaders.set("Content-Type", "application/json");
    const response = await authenticatedFetch(
      `/api/v1/config/sections/${activeConfigurationSection}`, {
      method: "PUT",
      headers: requestHeaders,
      body: JSON.stringify(request),
      },
...
async function loadMikanManualSeriesMappings(): Promise<void> {
  const container = element<HTMLElement>("#mikan-manual-series-mappings");
  setRegionState(container, "loading");
  try {
    const response = await authenticatedFetch(
    ...
    if (!response.ok) throw new Error(await responseError(response));
    const body = await response.json() as { items: MikanManualSeriesMappingItem[] };
    if (body.items.length === 0) {
    ...
    renderRegionMessage(
      container,
      "error",
      `人工 TMDB 映射读取失败：${errorMessage(error, "未知错误")}`,
    );
  }
}

async function deleteMikanManualSeriesMapping(
  item: MikanManualSeriesMappingItem,
): Promise<void> {
  if (!window.confirm(
    `删除 Mikan ${item.mikanid} / Group ${item.groupid} 的人工 TMDB 映射？之后的新任务会重新执行正常 TMDB 搜索与匹配。`,
  )) return;
  const response = await authenticatedFetch(
    `/api/v1/mikan/manual-series-mappings/${item.mikanid}/${item.groupid}`,
    { method: "DELETE", headers },
  );
  if (!response.ok) {
...
function pluginCallModeLabel(mode: MikanPluginCallLog["mode"]): string {
  if (mode === "single") return "单集";
  if (mode === "all") return "全集";
  if (mode === "selected") return "选集";
  return "批量单集";
}

async function loadMikanPluginCallLogs(): Promise<void> {
  const container = element<HTMLElement>("#mikan-plugin-call-list");
  setRegionState(container, "loading");
  const mode = element<HTMLSelectElement>("#mikan-plugin-call-mode").value;
  const result = element<HTMLSelectElement>("#mikan-plugin-call-result").value;
  const query = new URLSearchParams({ page: "1", page_size: "100" });
  if (mode) query.set("mode", mode);
  if (result) query.set("result", result);
  try {
    const response = await authenticatedFetch(
      `/api/v1/logs/mikan-plugin-calls?${query}`,
      { headers },
    );
    if (!response.ok) throw new Error(await responseError(response));
    const body = await response.json() as MikanPluginCallLogList;
    element<HTMLElement>("#mikan-plugin-call-count").textContent =
      ...
      if (item.items.length === 0) {
        ...
        item.items.forEach(detail => {
          ...
          [
            ...
            const cell = document.createElement("td");
            cell.textContent = value;
            if (columnIndex === 1 && detail.title !== null) cell.title = detail.title;
            row.append(cell);
          });
...
function notificationProviderChanged(resetEndpoint = false): void {
  const provider = element<HTMLSelectElement>("#notification-provider")
    .value as NotificationProvider;
  const endpoint = element<HTMLInputElement>("#notification-endpoint");
  const previous = endpoint.dataset.provider as NotificationProvider | undefined;
  ...
  element<HTMLElement>("#notification-bark-options").hidden = provider !== "bark";
  element<HTMLElement>("#notification-generic-options").hidden = provider !== "generic";

  const secretField = element<HTMLElement>("#notification-secret-field");
  const targetField = element<HTMLElement>("#notification-target-field");
  ...
  if (secretField.firstChild) secretField.firstChild.textContent = secretLabel;
  if (targetField.firstChild) {
    targetField.firstChild.textContent = provider === "telegram"
      ? "Chat ID" : "PushPlus topic（可空）";
  }
  element<HTMLInputElement>("#notification-secret").required =
    ["bark", "telegram", "serverchan", "pushplus"].includes(provider);
  element<HTMLInputElement>("#notification-target").required = provider === "telegram";
}

function resetNotificationForm(): void {
  element<HTMLFormElement>("#notification-channel-form").reset();
  element<HTMLInputElement>("#notification-channel-id").value = "";
  element<HTMLSelectElement>("#notification-provider").value = "bark";
  element<HTMLInputElement>("#notification-enabled").checked = true;
  element<HTMLInputElement>("#notification-bark-group").value = "AnimeGoNet";
  element<HTMLSelectElement>("#notification-bark-level").value = "active";
  element<HTMLTextAreaElement>("#notification-generic-template").value =
    '{"event":{{event_json}},"title":{{title_json}},"body":{{body_json}},"task_id":{{task_id_json}}}';
  document.querySelectorAll<HTMLInputElement>('input[name="notification-event"]')
    .forEach(input => {
      input.checked = [
        "metadata_failed",
        "metadata_other",
  ...
  element<HTMLButtonElement>("#notification-delete").disabled = true;
  element<HTMLButtonElement>("#notification-test").disabled = true;
  element<HTMLElement>("#notification-form-status").textContent = "新建通知渠道。";
}

function notificationOptionsFromForm(provider: NotificationProvider): JsonObject {
  if (provider === "bark") {
    ...
    options.level = element<HTMLSelectElement>("#notification-bark-level").value;
    const badge = element<HTMLInputElement>("#notification-bark-badge").valueAsNumber;
    if (Number.isFinite(badge)) options.badge = badge;
    options.auto_copy = element<HTMLInputElement>("#notification-bark-auto-copy").checked;
    return options;
  }
  if (provider === "generic") {
    const headersText = element<HTMLTextAreaElement>("#notification-generic-headers")
      .value.trim();
    return {
      headers: headersText ? jsonObject(JSON.parse(headersText), "额外请求头") : {},
      body_template: element<HTMLTextAreaElement>("#notification-generic-template").value,
    };
  }
  return {};
}

function notificationPayload(): JsonObject {
  const provider = element<HTMLSelectElement>("#notification-provider")
    .value as NotificationProvider;
  return {
    name: element<HTMLInputElement>("#notification-name").value.trim(),
    provider,
    enabled: element<HTMLInputElement>("#notification-enabled").checked,
    endpoint_url: element<HTMLInputElement>("#notification-endpoint").value.trim(),
    secret: element<HTMLInputElement>("#notification-secret").value.trim() || null,
    target: element<HTMLInputElement>("#notification-target").value.trim() || null,
    options: notificationOptionsFromForm(provider),
    events: [...document.querySelectorAll<HTMLInputElement>(
      'input[name="notification-event"]:checked',
    )].map(input => input.value),
  };
}

function notificationOptionText(options: JsonObject, key: string): string {
  const value = options[key];
  return typeof value === "string" ? value : "";
}

function selectNotificationChannel(channel: NotificationChannel): void {
  element<HTMLInputElement>("#notification-channel-id").value = channel.id;
  element<HTMLInputElement>("#notification-name").value = channel.name;
  element<HTMLSelectElement>("#notification-provider").value = channel.provider;
  notificationProviderChanged(true);
  element<HTMLInputElement>("#notification-endpoint").value = channel.endpoint_url;
  element<HTMLInputElement>("#notification-secret").value = channel.secret ?? "";
  element<HTMLInputElement>("#notification-target").value = channel.target ?? "";
  element<HTMLInputElement>("#notification-enabled").checked = channel.enabled;
  if (channel.provider === "bark") {
    element<HTMLInputElement>("#notification-bark-group").value =
      notificationOptionText(channel.options, "group");
    element<HTMLInputElement>("#notification-bark-sound").value =
      notificationOptionText(channel.options, "sound");
    element<HTMLInputElement>("#notification-bark-icon").value =
      notificationOptionText(channel.options, "icon");
    element<HTMLInputElement>("#notification-bark-url").value =
      notificationOptionText(channel.options, "url");
    element<HTMLSelectElement>("#notification-bark-level").value =
      notificationOptionText(channel.options, "level") || "active";
    const badge = channel.options.badge;
    element<HTMLInputElement>("#notification-bark-badge").value =
      typeof badge === "number" ? String(badge) : "";
    element<HTMLInputElement>("#notification-bark-copy").value =
      notificationOptionText(channel.options, "copy");
    element<HTMLInputElement>("#notification-bark-auto-copy").checked =
      channel.options.auto_copy === true;
  }
  if (channel.provider === "generic") {
    element<HTMLTextAreaElement>("#notification-generic-headers").value =
  ...
  element<HTMLButtonElement>("#notification-delete").disabled = false;
  element<HTMLButtonElement>("#notification-test").disabled = false;
  element<HTMLElement>("#notification-form-status").textContent =
    `正在编辑“${channel.name}”；敏感值已直接回填。`;
}

async function loadNotificationChannels(): Promise<void> {
  const container = element<HTMLElement>("#notification-channel-list");
  setRegionState(container, "loading");
  try {
    const response = await authenticatedFetch("/api/v1/notifications/channels", { headers });
    if (!response.ok) throw new Error(await responseError(response));
    notificationChannels = (await response.json() as { items: NotificationChannel[] }).items;
    ...
    renderRegionMessage(
      container,
      "error",
      `通知渠道读取失败：${errorMessage(error, "未知错误")}`,
    );
  }
}

async function saveNotificationChannel(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  try {
    const id = element<HTMLInputElement>("#notification-channel-id").value;
    const response = await authenticatedFetch(
      id
...
async function testNotificationChannel(): Promise<void> {
  const id = element<HTMLInputElement>("#notification-channel-id").value;
  if (!id) return;
  element<HTMLElement>("#notification-form-status").textContent = "正在发送测试通知…";
  try {
    const response = await authenticatedFetch(
    ...
    if (!response.ok) throw new Error(await responseError(response));
    const result = await response.json() as NotificationTestResult;
    element<HTMLElement>("#notification-form-status").textContent = result.succeeded
...
async function deleteNotificationChannel(): Promise<void> {
  const id = element<HTMLInputElement>("#notification-channel-id").value;
...
async function loadNotificationDeliveries(): Promise<void> {
  const container = element<HTMLElement>("#notification-delivery-list");
  setRegionState(container, "loading");
  try {
    const response = await authenticatedFetch(
      "/api/v1/notifications/deliveries?limit=100",
      { headers },
    );
    if (!response.ok) throw new Error(await responseError(response));
    const items = (await response.json() as { items: NotificationDelivery[] }).items;
    if (items.length === 0) {
    ...
    renderRegionMessage(
      container,
      "error",
      `发送记录读取失败：${errorMessage(error, "未知错误")}`,
    );
  }
}

function trustedOffsetBlacklistPayload(): JsonObject {
  const scope = element<HTMLSelectElement>("#trusted-offset-blacklist-scope")
    .value as MikanTrustedOffsetBlacklistScope;
  const mikanId = element<HTMLInputElement>("#trusted-offset-blacklist-mikanid")
    .valueAsNumber;
  const groupId = element<HTMLInputElement>("#trusted-offset-blacklist-groupid")
    .valueAsNumber;
  return {
    scope,
...
function updateTrustedOffsetBlacklistFields(): void {
  const scope = element<HTMLSelectElement>("#trusted-offset-blacklist-scope")
    .value as MikanTrustedOffsetBlacklistScope;
  const mikanField = element<HTMLElement>("#trusted-offset-blacklist-mikan-field");
  const groupField = element<HTMLElement>("#trusted-offset-blacklist-group-field");
  const mikanInput = element<HTMLInputElement>("#trusted-offset-blacklist-mikanid");
  const groupInput = element<HTMLInputElement>("#trusted-offset-blacklist-groupid");
...
function trustedOffsetBlacklistLabel(item: MikanTrustedOffsetBlacklistItem): string {
...
async function loadTrustedOffsetBlacklist(): Promise<void> {
  const container = element<HTMLElement>("#trusted-offset-blacklist");
  setRegionState(container, "loading");
  try {
    const response = await authenticatedFetch(
    ...
    if (!response.ok) throw new Error(await responseError(response));
    const body = await response.json() as { items: MikanTrustedOffsetBlacklistItem[] };
    if (body.items.length === 0) {
    ...
    renderRegionMessage(
      container,
      "error",
      `黑名单读取失败：${errorMessage(error, "未知错误")}`,
    );
  }
}

async function addTrustedOffsetBlacklist(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  try {
    const response = await authenticatedFetch(
      "/api/v1/mikan/trusted-offset-blacklist",
...
async function removeTrustedOffsetBlacklist(
  item: MikanTrustedOffsetBlacklistItem,
): Promise<void> {
  if (!window.confirm(
    `将 ${trustedOffsetBlacklistLabel(item)} 移出可信 Offset 黑名单？以后可以重新开始学习，但已清除的旧证据不会恢复。`,
  )) return;
  const query = new URLSearchParams({ scope: item.scope });
  if (item.mikanid !== null) query.set("mikanid", String(item.mikanid));
  if (item.groupid !== null) query.set("groupid", String(item.groupid));
  try {
    const response = await authenticatedFetch(
      `/api/v1/mikan/trusted-offset-blacklist?${query}`,
      { method: "DELETE", headers },
    );
    if (!response.ok) throw new Error(await responseError(response));
...
deleteConfirm.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(await responseError(response));
    const body = await response.json() as DeleteExecutionResponse;
    ...
    if (body.state === "completed") {
      element<HTMLElement>("#delete-message").textContent = `删除完成。${summary}`;
    ...
    element<HTMLElement>("#delete-message").textContent = errorMessage(error, "删除失败");
    deleteConfirm.textContent = "重试并等待结果";
    updateDeleteConfirm();
  }
});
...
function manualEpisodeCandidate(value: string | null): number | null {
...
function renderManualMetadataFiles(): void {
  const preview = activeManualMetadataPreview;
  if (!preview) return;
  const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value as "tv" | "movie";
  element<HTMLElement>("#manual-metadata-season-field").hidden = mediaType === "movie";
  const container = element<HTMLElement>("#manual-metadata-assignment-files");
  container.replaceChildren();
  ...
  for (const file of preview.files) {
    ...
    if (mediaType === "tv") {
      ...
      role.value = file.is_video
        ...
        ? "main"
        : "extras";
      const syncMovieRoleHighlight = (select: HTMLSelectElement) => {
      ...
      role.addEventListener("change", () => {
        if (role.value === "main") {
          container.querySelectorAll<HTMLSelectElement>(".manual-metadata-file-role")
            .forEach(other => {
              if (other !== role && other.value === "main") other.value = "extras";
...
function updateManualMetadataSearchMode(clearIdentity = false): void {
  const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value as "tv" | "movie";
  element<HTMLElement>("#manual-metadata-query-label").textContent = mediaType === "tv"
    ? "TV 名称"
    : "Movie 名称";
  element<HTMLButtonElement>("#manual-metadata-search").textContent = mediaType === "tv"
    ? "搜索 TMDB TV"
    : "搜索 TMDB Movie";
  element<HTMLElement>("#manual-metadata-search-results").replaceChildren();
  renderAnitomyTitleCandidates(
    element<HTMLElement>("#manual-metadata-anitomy-preview"),
    null,
    "",
    () => undefined,
  );
  if (clearIdentity) {
    element<HTMLInputElement>("#manual-metadata-tmdb-id").value = "";
    element<HTMLInputElement>("#manual-metadata-season").value = "";
  }
}

function renderManualAnitomyTitle(result: AnitomyTitleParseResult | null, message = ""): void {
  renderAnitomyTitleCandidates(
    element<HTMLElement>("#manual-metadata-anitomy-preview"),
    result,
    message,
    title => {
      const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value;
      const normalized = mediaType === "movie"
        ? normalizeMixedMediaMovieSearchTitle(title)
        : title.trim();
      element<HTMLInputElement>("#manual-metadata-title-query").value = normalized || title;
      element<HTMLElement>("#manual-metadata-search-results").replaceChildren();
    },
  );
}

function manualMetadataTitleSource(): string {
  const preview = activeManualMetadataPreview;
  return preview?.title ?? "";
}

async function parseManualMetadataTitle(): Promise<void> {
  const sourceText = manualMetadataTitleSource();
  if (!sourceText) return;
  const button = element<HTMLButtonElement>("#manual-metadata-anitomy-parse");
  button.disabled = true;
  renderManualAnitomyTitle(null, "AnitomySharp 正在解析标题…");
  try {
    const response = await authenticatedFetch(
    ...
    renderManualAnitomyTitle(result);
    if (result.success && result.anime_title) {
      const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value;
      const normalized = mediaType === "movie"
        ? normalizeMixedMediaMovieSearchTitle(result.anime_title)
        : result.anime_title.trim();
      element<HTMLInputElement>("#manual-metadata-title-query").value = normalized || result.anime_title;
      element<HTMLElement>("#manual-metadata-search-results").replaceChildren();
    }
  } catch (error) {
    renderManualAnitomyTitle(null, errorMessage(error, "AnitomySharp 解析失败"));
  } finally {
    button.disabled = false;
  }
}

function manualMetadataSearchQueries(primary: string): string[] {
  const preview = activeManualMetadataPreview;
  const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value;
  const normalize = (value: string) => mediaType === "movie"
    ? normalizeMixedMediaMovieSearchTitle(value)
    : value.trim();
  ...
  const append = (value: string): void => {
    ...
    if (normalized.length < 2 || normalized.length > 256 || seen.has(key)) return;
    seen.add(key);
    queries.push(normalized);
  };
  append(primary);
  if (!preview) return queries;
  for (const source of [preview.title, ...preview.files.map(file => file.relative_path)]) {
    for (const match of source.matchAll(/\[([^\[\]]{2,256})\]/gu)) {
      const candidate = match[1].trim();
      if (isMixedMediaTechnicalTitle(candidate)) continue;
      append(candidate);
...
async function searchManualMetadataTitles(): Promise<void> {
  const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value as "tv" | "movie";
  const query = element<HTMLInputElement>("#manual-metadata-title-query").value.trim();
  const results = element<HTMLElement>("#manual-metadata-search-results");
  const message = element<HTMLElement>("#manual-metadata-assignment-message");
  if (!query) {
    message.textContent = "请先填写或解析作品名称。";
    return;
  }
  const searchButton = element<HTMLButtonElement>("#manual-metadata-search");
  searchButton.disabled = true;
  results.replaceChildren();
  message.textContent = `正在搜索 TMDB ${mediaType === "tv" ? "TV" : "Movie"}…`;
  try {
    const queries = manualMetadataSearchQueries(query);
    ...
    for (const candidate of queries) {
      ...
      const response = await authenticatedFetch(
        `/api/v1/tmdb/${endpoint}/search?query=${encodeURIComponent(candidate)}`,
        { headers },
      );
      if (!response.ok) throw new Error(await responseError(response));
      if (mediaType === "tv") {
        const body = await response.json() as { items: TmdbSeriesSearchItem[] };
        candidates = body.items.map(item => ({
          id: item.tmdb_series_id,
          title: item.name,
          originalTitle: item.original_name,
          date: item.first_air_date,
        }));
      } else {
        const body = await response.json() as { items: TmdbMovieSearchItem[] };
        candidates = body.items.map(item => ({
    ...
    message.textContent = candidates.length === 0
      ? `TMDB 没有返回候选；已尝试 ${queries.length} 个任务内标题。`
      : matchedQuery === query
        ? "请选择一个候选，TMDB ID 会自动回填。"
        : `“${query}”没有结果，已通过任务内标题“${matchedQuery}”找到候选。`;
    for (const candidate of candidates) {
      const row = document.createElement("span");
      row.className = "manual-metadata-search-result-row";
      const select = document.createElement("button");
      select.type = "button";
      select.className = "manual-metadata-search-result";
      const marker = document.createElement("span");
      marker.textContent = "○";
      const text = document.createElement("span");
      const title = document.createElement("strong");
      title.textContent = `${candidate.title} · TMDB ${candidate.id}`;
      const detail = document.createElement("small");
      detail.textContent = `${candidate.originalTitle || "无原名"} · ${mediaType === "tv" ? "开播" : "上映"} ${candidate.date ?? "未知"}`;
      text.append(title, document.createElement("br"), detail);
      select.append(marker, text);
      select.addEventListener("click", () => {
        results.querySelectorAll<HTMLElement>(".manual-metadata-search-result")
          .forEach(item => {
            item.classList.remove("selected");
            if (item.firstElementChild) item.firstElementChild.textContent = "○";
          });
        select.classList.add("selected");
        marker.textContent = "●";
        element<HTMLInputElement>("#manual-metadata-tmdb-id").value = String(candidate.id);
        message.textContent = `已选择 ${candidate.title} · TMDB ${candidate.id}。`;
      });
      const link = document.createElement("a");
      link.className = "secondary-button mixed-media-tmdb-link";
      link.href = `https://www.themoviedb.org/${mediaType === "tv" ? "tv" : "movie"}/${candidate.id}`;
      link.target = "_blank";
      link.rel = "noopener noreferrer";
      link.textContent = "打开 TMDB ↗";
      link.setAttribute("aria-label", `在 TMDB 打开 ${candidate.title}`);
      row.append(select, link);
      results.append(row);
    }
  } catch (error) {
    message.textContent = errorMessage(error, "TMDB 搜索失败");
  } finally {
    searchButton.disabled = false;
  }
}

async function openManualMetadataAssignment(taskId: string): Promise<void> {
  const message = element<HTMLElement>("#manual-metadata-assignment-message");
  ...
  try {
    const response = await authenticatedFetch(
      `/api/v1/metadata/tasks/${encodeURIComponent(taskId)}/manual-assignment`,
      { headers },
    );
    if (!response.ok) throw new Error(await responseError(response));
    const preview = await response.json() as ManualMetadataAssignmentPreview;
    activeManualMetadataPreview = preview;
    element<HTMLElement>("#manual-metadata-assignment-summary").textContent =
      `${preview.title} · ${statusLabels[preview.status] ?? preview.status} · ${preview.files.length} 个文件`;
    const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type");
    ...
    element<HTMLInputElement>("#manual-metadata-tmdb-id").value = String(
      preview.media_type === "movie"
        ? movieIdentity?.tmdb_movie_id ?? ""
        : tvIdentity?.tmdb_series_id ?? "",
    );
    element<HTMLInputElement>("#manual-metadata-season").value = String(
      tvIdentity?.tmdb_season_number ?? "",
    );
    element<HTMLInputElement>("#manual-metadata-title-query").value = preview.title;
    renderManualMetadataFiles();
    updateManualMetadataSearchMode();
    await parseManualMetadataTitle();
...
async function submitManualMetadataAssignment(): Promise<void> {
  const preview = activeManualMetadataPreview;
  if (!preview?.eligible) return;
  const message = element<HTMLElement>("#manual-metadata-assignment-message");
  const mediaType = element<HTMLSelectElement>("#manual-metadata-media-type").value as "tv" | "movie";
  const tmdbId = Number(element<HTMLInputElement>("#manual-metadata-tmdb-id").value);
  const seasonNumber = mediaType === "tv"
    ? Number(element<HTMLInputElement>("#manual-metadata-season").value)
    : null;
  const rows = Array.from(element<HTMLElement>("#manual-metadata-assignment-files")
    .querySelectorAll<HTMLElement>(".manual-metadata-file-row"));
  const files = rows.map(row => {
    const role = row.querySelector<HTMLSelectElement>(".manual-metadata-file-role")!.value;
    const episodeInput = row.querySelector<HTMLInputElement>(".manual-metadata-file-episode");
    return {
  ...
  if (!window.confirm(`确认手动覆盖该任务的自动匹配结果？\n${description}\n提交后将重新进入下载/整理流程。`)) return;
  manualMetadataConfirm.disabled = true;
  message.textContent = "正在验证 TMDB 并原子写入文件归类…";
  try {
    const response = await authenticatedFetch(
      `/api/v1/metadata/tasks/${encodeURIComponent(preview.task_id)}/manual-assignment`,
      {
        method: "POST",
        headers: jsonRequestHeaders(),
        body: JSON.stringify({
...
function recommendedMixedMediaMainId(
  preview: MixedMediaPostprocessPreview,
): string | null {
  const candidates = preview.files.filter(file =>
    file.source_available
    && (preview.media_type === "movie"
      ? file.movie_role === "extras"
      : file.movie_hint && file.movie_role === null));
...
interface AnitomyTitleCandidateRange {
  start: number;
  length: number;
  title: string;
}

function anitomyTitleCandidateRanges(result: AnitomyTitleParseResult): AnitomyTitleCandidateRange[] {
  const ranges: AnitomyTitleCandidateRange[] = [];
  for (const match of result.source_text.matchAll(/\[([^\[\]]{2,256})\]/gu)) {
    if (match.index === undefined) continue;
    const title = match[1].trim();
    if (isMixedMediaTechnicalTitle(title)) continue;
    const leadingWhitespace = match[1].length - match[1].trimStart().length;
    ranges.push({
      start: match.index + 1 + leadingWhitespace,
      length: title.length,
      title,
    });
  }
  const primaryValid = result.success
    && result.anime_title
    && result.match_start >= 0
    && result.match_length > 0
    && result.match_start + result.match_length <= result.source_text.length;
  if (primaryValid && !ranges.some(range =>
    result.match_start >= range.start
    && result.match_start + result.match_length <= range.start + range.length)) {
    ranges.push({
      start: result.match_start,
      length: result.match_length,
      title: result.source_text.slice(result.match_start, result.match_start + result.match_length),
    });
  }
  return ranges
    .sort((left, right) => left.start - right.start || right.length - left.length)
    .filter((range, index, all) => index === 0
      || range.start >= all[index - 1].start + all[index - 1].length)
    .slice(0, 8);
}

function renderAnitomyTitleCandidates(
  preview: HTMLElement,
  result: AnitomyTitleParseResult | null,
  message: string,
  ...
  const ranges = anitomyTitleCandidateRanges(result);
  let cursor = 0;
  for (const [index, range] of ranges.entries()) {
    if (range.start > cursor) {
      source.append(document.createTextNode(result.source_text.slice(cursor, range.start)));
    }
    const candidate = document.createElement("button");
    candidate.type = "button";
    candidate.className = "anitomy-title-candidate";
    candidate.dataset.color = String(index % 4);
    candidate.textContent = result.source_text.slice(range.start, range.start + range.length);
    candidate.title = `使用“${range.title}”搜索 TMDB`;
    candidate.addEventListener("click", () => selectTitle(range.title));
    source.append(candidate);
    cursor = range.start + range.length;
  }
  if (cursor < result.source_text.length) {
    source.append(document.createTextNode(result.source_text.slice(cursor)));
  }
  if (ranges.length === 0) source.textContent = result.source_text || "—";
  preview.append(label, source);
}

function renderAnitomyMovieTitle(result: AnitomyTitleParseResult | null, message = ""): void {
  renderAnitomyTitleCandidates(
    element<HTMLElement>("#mixed-media-anitomy-preview"),
    result,
    message,
    title => {
      const normalized = normalizeMixedMediaMovieSearchTitle(title);
      element<HTMLInputElement>("#mixed-media-movie-query").value = normalized || title;
      selectedMixedMediaMovie = null;
...
async function openMixedMediaPostprocess(taskId: string): Promise<void> {
  ...
  const summary = element<HTMLElement>("#mixed-media-postprocess-summary");
  const files = element<HTMLElement>("#mixed-media-postprocess-files");
  const results = element<HTMLElement>("#mixed-media-movie-results");
  message.textContent = "";
  ...
  try {
    const response = await authenticatedFetch(
      `/api/v1/metadata/tasks/${encodeURIComponent(taskId)}/mixed-media-postprocess/preview`,
      { headers },
    );
    if (!response.ok) throw new Error(await responseError(response));
    const preview = await response.json() as MixedMediaPostprocessPreview;
    activeMixedMediaPreview = preview;
    element<HTMLElement>("#mixed-media-postprocess-title").textContent = preview.media_type === "movie"
      ? "多 Movie 合集后处理"
      : "TV+Movie 后处理";
    element<HTMLElement>("#mixed-media-postprocess-purpose").textContent = preview.media_type === "movie"
      ? "此功能用于处理包含多部电影的 Movie 合集：保留当前 Movie，把选中的另一部 Movie 正片及其 Extras 分离并迁移为独立 Movie。"
      : "此功能用于处理 TV+Movie 合集：把合集中的 Movie 正片及其 Extras 从 TV 内容中分离，并迁移为独立的 Movie 类型。";
    summary.textContent = preview.eligible
      ? preview.mode === "edit_pending"
        ? `${preview.title} · 后处理尚未开始整理，可再次指定哪一个是 Movie 正片、哪些属于 Movie Extras。`
        : preview.media_type === "movie"
          ? `${preview.title} · 当前 Movie 主文件保持不变；请选择另一部 Movie 正片及其 Extras，再确认新的 TMDB Movie。`
          : `${preview.title} · 请逐个指定保留 TV、Movie 正片或 Movie Extras，再确认独立的 TMDB Movie。`
      : `${preview.title} · ${preview.reason ?? "当前不可执行后处理"}`;
    const recommendedMainId = recommendedMixedMediaMainId(preview);
    for (const file of preview.files) {
      ...
      for (const [value, text] of [
        ["keep", preview.media_type === "movie" ? "保留当前 Movie / 不迁移" : "保留 TV / 不迁移"],
        ["movie", "Movie 正片"],
        ["extras", "Movie Extras"],
      ...
      const pendingUnselected = preview.mode === "edit_pending" && !file.pending_postprocess;
      const protectedExistingMovie = preview.mode === "create"
        && file.movie_role !== null
        && (preview.media_type === "tv" || file.movie_role === "movie");
      select.disabled = !preview.eligible || !file.source_available || pendingUnselected || protectedExistingMovie;
      select.value = protectedExistingMovie
        ? "keep"
        : file.pending_postprocess
          ? file.movie_role ?? "keep"
      ...
      detail.textContent = `${readaptationDisposition(file.disposition)} · ${formatBytes(file.size_bytes)} · `
        ...
        + `${protectedExistingMovie ? " · 当前 Movie 主文件必须保留" : ""}`
...
async function searchMixedMediaMovies(): Promise<void> {
  const query = element<HTMLInputElement>("#mixed-media-movie-query").value.trim();
  const results = element<HTMLElement>("#mixed-media-movie-results");
  const message = element<HTMLElement>("#mixed-media-postprocess-message");
  if (!query) {
  ...
  updateMixedMediaConfirmState();
  try {
    const queries = mixedMediaMovieSearchQueries(query);
    let body: { items: TmdbMovieSearchItem[] } = { items: [] };
    let matchedQuery = query;
    for (const candidate of queries) {
      const response = await authenticatedFetch(
        `/api/v1/tmdb/movies/search?query=${encodeURIComponent(candidate)}`,
        { headers },
      );
      if (!response.ok) throw new Error(await responseError(response));
      body = await response.json() as { items: TmdbMovieSearchItem[] };
      if (body.items.length > 0) {
        matchedQuery = candidate;
        break;
      }
    ...
    message.textContent = body.items.length === 0
      ? `TMDB 没有返回 Movie 候选；已尝试 ${queries.length} 个任务内标题。`
      : matchedQuery === query
        ? "请选择一个候选；提交时后端会再次按 ID 验证。"
        : `“${query}”没有结果，已通过任务内标题“${matchedQuery}”找到候选；提交时后端会再次按 ID 验证。`;
    for (const movie of body.items) {
      const row = document.createElement("span");
      row.className = "mixed-media-movie-result-row";
      const button = document.createElement("button");
      button.type = "button";
      ...
      button.addEventListener("click", () => {
        results.querySelectorAll<HTMLElement>(".mixed-media-movie-result")
          .forEach(item => {
            item.classList.remove("selected");
            const itemMarker = item.firstElementChild;
            if (itemMarker) itemMarker.textContent = "○";
          });
      ...
      const tmdbLink = document.createElement("a");
      tmdbLink.className = "secondary-button mixed-media-tmdb-link";
      tmdbLink.href = `https://www.themoviedb.org/movie/${movie.tmdb_movie_id}`;
      tmdbLink.target = "_blank";
      tmdbLink.rel = "noopener noreferrer";
      tmdbLink.textContent = "打开 TMDB ↗";
      tmdbLink.setAttribute("aria-label", `在 TMDB 打开 ${movie.title}`);
      row.append(button, tmdbLink);
      results.append(row);
    }
  } catch (error) {
...
function mixedMediaMovieSearchQueries(primary: string): string[] {
  const queries: string[] = [];
  const seen = new Set<string>();
  const append = (value: string): void => {
    const normalized = normalizeMixedMediaMovieSearchTitle(value);
    const key = normalized.toLocaleLowerCase();
    if (normalized.length < 2 || normalized.length > 256 || seen.has(key)) return;
    seen.add(key);
    queries.push(normalized);
  ...
  const sources = [
    preview.title,
    ...preview.files
      .filter(file => file.task_file_id === mainId)
      .map(file => file.source_name),
  ];
...
function isMixedMediaTechnicalTitle(value: string): boolean {
  const compact = value.trim().replace(/[\s_.-]+/gu, "").toLocaleLowerCase();
  return /^(?:\d{3,4}p|\d{3,4}x\d{3,4}|(?:bd|web|dvd|hd|tv)rip|bluray|h26[45]|hevc|avc|x26[45]|aac|flac\d*|atmos|mkv|mp4|movie\+?sp|sp|fin)$/iu.test(compact)
    || /^(?:chs|cht|jpn|eng|简|繁|简繁|内封|外挂|自压|付相关专辑)$/iu.test(compact)
    || /(?:bdrip|webrip|bluray|\b(?:h26[45]|hevc|avc|x26[45]|aac|flac|atmos|mkv|mp4)\b)/iu.test(value)
    || /(?:字幕组|字幕社|压制组|[-_. ]raws?)$/iu.test(value);
}
...
async function confirmMixedMediaPostprocess(): Promise<void> {
  ...
  if (preview.mode === "readonly") return;
  const message = element<HTMLElement>("#mixed-media-postprocess-message");
  ...
  try {
    ...
    requestHeaders.set("Content-Type", "application/json");
    const response = await authenticatedFetch(
      `/api/v1/metadata/tasks/${encodeURIComponent(preview.task_id)}/mixed-media-postprocess`,
      {
        method: "POST",
        headers: requestHeaders,
        body: JSON.stringify({
    ...
    message.textContent = errorMessage(error, "合集后处理失败");
    updateMixedMediaConfirmState();
  }
}
...
function renderMetadataAttentionSummary(
  ...
  element<HTMLElement>("#overview-attention-other-count").textContent =
    attention.other_items.toLocaleString("zh-CN");
  element<HTMLElement>("#overview-attention-failed-count").textContent =
    attention.failed_items.toLocaleString("zh-CN");
  element<HTMLElement>("#overview-attention-review-count").textContent =
...
function setMetadataAttentionFilter(filter: "other" | "failed" | "review"): void {
...
function openMetadataAttentionFromOverview(filter: "other" | "failed" | "review"): void {
  setMetadataAttentionFilter(filter);
  selectWorkspace("tasks", "metadata");
}
...
async function loadMatchingLogs(): Promise<void> {
  ...
  try {
    ...
    const cards = body.items.map(item => {
      ...
      flow.append(
        matchingLogStage(item, "Series", "series",
          movieOnly ? item.movie_strategy : item.series_strategy,
          movieOnly ? item.movie_run_id : item.series_run_id,
          movieOnly ? item.movie_attempt_id : item.series_attempt_id,
          movieOnly
            ? item.tmdb_movie_id === null ? "Movie 身份未记录" : `TMDB Movie ${item.tmdb_movie_id}`
            : item.tmdb_series_id === null ? null : `TMDB ${item.tmdb_series_id}`),
        matchingLogStage(item, "Season", "season",
          movieOnly ? null : item.season_strategy,
          movieOnly ? null : item.season_run_id,
          movieOnly ? null : item.season_attempt_id,
          movieOnly
        ...
        matchingLogStage(item, "Episode", "episode", item.episode_strategy, item.episode_run_id, item.episode_attempt_id,
          movieOnly ? "不适用（电影）" : episodeIdentity),
      );
...
async function loadMetadataTasks(background = false): Promise<void> {
  ...
  try {
    ...
    const cards = body.items.map((item) => {
      ...
      mixed.textContent = "合集后处理";
      const mixedMediaApplicable = item.status === "organized"
        && (item.tmdb_series_id !== null || item.movie_file_count > 0);
      mixed.disabled = !mixedMediaApplicable;
      mixed.title = mixedMediaApplicable
        ? item.tmdb_series_id !== null
          ? "把已整理 TV 合集中的 Movie 正片及 Extras 迁移为独立 Movie"
          : "把多 Movie 合集中的另一部 Movie 正片及 Extras 拆分为独立 Movie"
        : item.status !== "organized"
          ? "仅已整理入库的 TV 或 Movie 任务可使用合集后处理"
          : "当前任务没有可用于合集后处理的媒体身份";
...
function updateSourceCredentialInputs(): void {
  ...
  element<HTMLElement>("#source-media-type-state").textContent = isU2
    ? "油猴脚本每次人工确认的 TV/Movie 优先；这里是其他 U2 调用未明确传入类型时的默认值。"
    : isMikan
    ? "Mikan 手动触发和自动调度使用该类型；电影会走 TMDB Movie 并保存到电影目录。"
    : "调用未明确传入媒体类型时使用该默认值。";
  const filterEnabled = element<HTMLInputElement>("#source-filter-enabled");
  const priorityEnabled = element<HTMLInputElement>("#source-priority-enabled");
  filterEnabled.disabled = !isMikan;
  priorityEnabled.disabled = !isMikan;
  if (!isMikan) {
    filterEnabled.checked = false;
    priorityEnabled.checked = false;
    element<HTMLInputElement>("#route-mikanid").value = "";
    element<HTMLInputElement>("#route-bgmid").value = "";
  }
...
function populateSourceForm(
  ...
  templateAdapter: "mikan" | "u2" = "mikan",
): void {
  ...
  element<HTMLInputElement>("#source-name").value = profile?.display_name ?? (isU2Template ? "U2" : "");
  adapter.value = profile?.adapter ?? templateAdapter;
  element<HTMLInputElement>("#source-downloader").value =
  ...
  category.title = categoryLock
    ...
    : "";
  element<HTMLInputElement>("#source-tags").value =
  ...
  element<HTMLTextAreaElement>("#source-hosts").value =
...
function aiTesterPayloadDisclosure(label: string, content: string): HTMLDetailsElement {
  const details = document.createElement("details");
...
function appendAiTesterApiRequest(item: AiTesterApiRequestEntry): void {
  ...
  card.append(
    header,
    aiTesterPayloadDisclosure("完整请求 Content", item.content),
  );
  target.append(card);
}
...
function updateAiTesterSourceStates(): void {
  ...
  const bgmEnabled = element<HTMLInputElement>("#ai-test-enable-bgm-mcp").checked;
  const anidbEnabled = element<HTMLInputElement>("#ai-test-enable-anidb").checked;
...
element<HTMLButtonElement>("#overview-attention-other").addEventListener(
  "click",
  () => openMetadataAttentionFromOverview("other"),
);
element<HTMLButtonElement>("#overview-attention-failed").addEventListener(
  "click",
  () => openMetadataAttentionFromOverview("failed"),
);
element<HTMLButtonElement>("#overview-attention-review").addEventListener(
  "click",
...
element<HTMLButtonElement>("#library-detail-mikan-completion").addEventListener(
  "click",
  openMikanSeasonCompletion,
);
...
element<HTMLButtonElement>("#library-detail-manual-assignment").addEventListener(
  "click",
  openLibraryManualAssignment,
);
...
element<HTMLButtonElement>("#mikan-season-completion-close").addEventListener(
  "click",
  closeMikanSeasonCompletion,
);
...
element<HTMLSelectElement>("#mikan-season-completion-binding").addEventListener(
  "change",
  () => void discoverMikanSeasonCompletionGroups(),
);
element<HTMLButtonElement>("#mikan-season-completion-preview").addEventListener(
  "click",
  () => void previewMikanSeasonCompletion(),
);
element<HTMLButtonElement>("#mikan-season-completion-default").addEventListener(
  "click",
  () => setMikanSeasonCompletionSelection("default"),
);
element<HTMLButtonElement>("#mikan-season-completion-all").addEventListener(
  "click",
  () => setMikanSeasonCompletionSelection("all"),
);
element<HTMLButtonElement>("#mikan-season-completion-none").addEventListener(
  "click",
  () => setMikanSeasonCompletionSelection("none"),
);
element<HTMLButtonElement>("#mikan-season-completion-confirm").addEventListener(
  "click",
  () => void confirmMikanSeasonCompletion(),
);
...
element<HTMLButtonElement>("#mikan-manual-series-mappings-reload").addEventListener(
  "click",
  () => void loadMikanManualSeriesMappings(),
);
element<HTMLButtonElement>("#mikan-plugin-call-reload").addEventListener(
  "click",
  () => void loadMikanPluginCallLogs(),
);
...
element<HTMLFormElement>("#trusted-offset-blacklist-form").addEventListener(
  "submit",
  event => void addTrustedOffsetBlacklist(event),
);
element<HTMLSelectElement>("#notification-provider").addEventListener(
  "change",
  () => notificationProviderChanged(),
);
element<HTMLFormElement>("#notification-channel-form").addEventListener(
  "submit",
  event => void saveNotificationChannel(event),
);
element<HTMLButtonElement>("#notification-new").addEventListener(
  "click",
  resetNotificationForm,
);
element<HTMLButtonElement>("#notification-reload").addEventListener(
  "click",
  () => void loadNotificationChannels(),
);
element<HTMLButtonElement>("#notification-test").addEventListener(
  "click",
  () => void testNotificationChannel(),
);
element<HTMLButtonElement>("#notification-delete").addEventListener(
  "click",
  () => void deleteNotificationChannel(),
);
element<HTMLButtonElement>("#notification-deliveries-reload").addEventListener(
  "click",
...
element<HTMLButtonElement>("#deployment-data-path-reload").addEventListener(
  "click",
  () => void loadDeploymentDataPath(),
);
element<HTMLFormElement>("#deployment-data-path-form").addEventListener(
  "submit",
  (event) => void saveDeploymentDataPath(event),
);
element<HTMLButtonElement>("#webui-authentication-reload").addEventListener(
  "click",
  () => void loadWebUiAuthentication(),
);
element<HTMLFormElement>("#webui-authentication-form").addEventListener(
  "submit",
...
element<HTMLButtonElement>("#configuration-edit-paths").addEventListener(
  "click",
  () => openConfigurationEditor("paths"),
);
element<HTMLButtonElement>("#configuration-edit-network").addEventListener(
  "click",
  () => openConfigurationEditor("network"),
);
element<HTMLButtonElement>("#configuration-edit-ai").addEventListener(
  "click",
  () => openConfigurationEditor("ai"),
);
...
element<HTMLButtonElement>("#source-new").addEventListener(
  "click",
  () => populateSourceForm(null, "mikan"),
);
element<HTMLButtonElement>("#source-new-u2").addEventListener(
  "click",
  () => populateSourceForm(null, "u2"),
);
element<HTMLButtonElement>("#source-u2-open-plugin").addEventListener(
  "click",
  () => selectWorkspace("plugins", "internal"),
);
element<HTMLButtonElement>("#source-u2-open-logs").addEventListener(
  "click",
  () => selectWorkspace("sources", "u2-plugin-calls"),
);
...
element<HTMLButtonElement>("#mixed-media-postprocess-close").addEventListener(
  "click",
  () => mixedMediaPostprocessDialog.close(),
);
element<HTMLButtonElement>("#manual-metadata-assignment-close").addEventListener(
  "click",
  () => manualMetadataDialog.close(),
);
element<HTMLButtonElement>("#manual-metadata-assignment-cancel").addEventListener(
  "click",
  () => manualMetadataDialog.close(),
);
element<HTMLSelectElement>("#manual-metadata-media-type").addEventListener(
  "change",
  () => {
    renderManualMetadataFiles();
    updateManualMetadataSearchMode(true);
    if (activeManualMetadataPreview) {
      element<HTMLInputElement>("#manual-metadata-title-query").value =
        activeManualMetadataPreview.title;
    }
  },
);
element<HTMLButtonElement>("#manual-metadata-anitomy-parse").addEventListener(
  "click",
  () => void parseManualMetadataTitle(),
);
element<HTMLButtonElement>("#manual-metadata-search").addEventListener(
  "click",
  () => void searchManualMetadataTitles(),
);
manualMetadataConfirm.addEventListener(
  "click",
  () => void submitManualMetadataAssignment(),
);
element<HTMLButtonElement>("#mixed-media-postprocess-cancel").addEventListener(
  "click",
  () => mixedMediaPostprocessDialog.close(),
);
...
mixedMediaPostprocessConfirm.addEventListener(
  "click",
  () => void confirmMixedMediaPostprocess(),
);
