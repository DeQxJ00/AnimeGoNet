...
任务从重新适配开始进入 `readaptation_review_state=pending`。schema v49 同时在逐文件作业中固化适配前的归类、Other 原因和 TMDB Series/Season/Episode。只有完成整理回到 `organized` 后，WebUI 的“确认人工审核”才可提交；点击后先由只读审核 API 返回前后对照。应用内弹窗为每个文件生成独立的“信息项 / 适配前 / 适配后”表格，归类、TMDB 名称/编号、原因、原/新媒体位置、Episode 取得策略以及共享源文件是否采用复制均分行显示；多文件和小屏幕在弹窗内部滚动，不再使用浏览器原生大段文字确认框。用户在该对照弹窗中再次确认后才写入审核通过。旧批次没有固化的字段显示为未知，不使用当前值反推历史状态。

审核前后端均拒绝删除 AnimeGoNet 任务记录。删除中心的独立“任务记录”选项始终最后执行；它不会替代业务记录、下载器任务、源文件和媒体文件四类选择。

底层任务状态不为展示效果新增伪状态：文件整理完成后仍为 `organized`，审核状态独立保存。任务卡将两者组合显示为“重新适配待审核”或“重新适配审核完成”；审核 API 另返回 `processing / awaiting_review / review_completed`，并显示执行完成、审核完成时间与最终正片/Other/重复目标统计。审核通过后可从任务卡再次只读打开最终对照，不会重复提交审核。
