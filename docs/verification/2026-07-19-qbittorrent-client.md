...
- qBittorrent WebUI API login with exact-origin Referer and cookie-capable `HttpClientHandler` per instance.
...
AnimeGoNet.App.Tests: 15 passed
...
The adapter is registered but ingest does not dispatch to it yet. Torrent URL security validation, bencode parsing, staging, info-hash acquisition, episode claim/dedup, and durable download-job creation must precede dispatch. Real qBittorrent container, reconnect, polling, file-priority, and path-mapping tests remain pending because Docker is unavailable on this host.
