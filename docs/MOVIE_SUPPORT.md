...
6. TV+Movie 合集不在下载前拆分。按 TV 完成后，可在“匹配与整理 → TV+Movie 后处理”
   对每个实际视频文件明确选择“保留 TV / Movie 正片 / Movie Extras”，通过可视化 TMDB
   Movie 搜索人工确认身份，并在执行前复核完整方案，再从 TV 媒体库迁移到
   `movie_save_path`。关键词只预选文件，后端仍会再次验证 TMDB Movie ID；必须且只能指定
   一个 Movie 正片，Extras 保留原名放入同一 Movie 目录。命中剧场版/Gekijouban/Movie 关键词的候选
   中以最大文件预选正片，其余预选为 Extras；Movie 名称默认采用 AnitomySharp 从正片文件名
   提取的标题，并在填入搜索框前移除 `劇場版`、`剧场版`、大小写任意的 `Gekijouban`
   和独立单词 `movie`；原始文件名中的标题片段仍会高亮展示；文件候选列表中的上述四个
   Movie 关键词也会逐处
   高亮，但不修改真实文件名。用户可一键重解析或手工改名。后台尚未
   取得整理租约时可再次编辑正片/Extras 角色及 TMDB Movie；整理开始后锁定方案。
