...
文件级选择下载只适用于 Mikan：Mikan 的 `duplicate` 与 `ignored` 文件设置为 priority 0，`episode` 与 `other` 文件设置为 priority 1；只有至少一个 wanted 文件时才恢复任务并进入 `download_queued`。U2/PT 等非 Mikan 来源必须保持完整下载和做种内容，下载准备不会按 Episode、重复或附件分类裁剪 Torrent，而是把 qB 返回的全部文件（包括可见的 `.pad`）统一恢复为 priority 1；分类只用于后续匹配与整理。Torrent 中通过 `attr=p`、`.pad` 路径或兼容 `_____padding_file` 路径标记的 padding file 是分片对齐数据：匹配时忽略，不进入 Extras，也不计入 EP/附件统计，但仍属于 U2/PT 的完整下载内容；qB 隐藏的 padding 保持客户端默认下载行为。下载准备会同时修复旧库中曾被误记为附件的 `.pad` 行。Mikan 全部实际文件都被去重时不调用恢复，持久化 `download_skipped_duplicate`，并以 `deleteFiles=false` 尝试移除 qB 任务。文件元数据尚未就绪、实际文件清单不一致、下载器离线或请求失败均保留 paused 语义，通过 SQLite preparation lease、attempt 和 next-attempt 安全重试；进程崩溃后的过期租约可恢复。
