...
U2 已交付人工按钮链：AnimeGoHelper 从 U2 表格读取条目，经 `inner_plugin_u2`
专用 API 进入统一导入；U2 RSS、站点登录和自动抓取仍不在本次范围。
...
| 不同来源绑定不同 qB 与规则 | revision 化 SourceProfile 保存 downloader、规则开关、策略/category/tags/做种 | CRUD、route preview、双实例隔离与快照测试；U2 手动插件按显式 SourceProfile 路由 |
