...
首版约定范围内已经没有未开始、进行中或阻塞的实现项。`TODO.md` 中剩余 `[~]`
全部表示代码、工作流或门禁已经生成，但没有把 Docker、远端原生 runner、外部数据
仓库或真实 GitHub Release 声称为已验证。U2/TTG 已由所有者明确暂缓，不属于首版
正式输入源。
...
| GitHub Actions 构建/测试/AOT/Docker | `dotnet-ci.yml`、`animegonet-native-aot.yml`、`animegonet-docker.yml` | YAML/交付契约测试；Docker 按所有者要求不在本机执行 |
...
| Docker 三路径与共享挂载一致 | `/data`、`/download/incomplete`、`/download/anime` 固定配置和 Compose 卷 | 配置/Compose/脚本契约；容器实跑按所有者要求 `[~]` |
...
| `mikanid` 人工覆盖与可信 offset | mikanid 规则最高优先；`mikanid+groupid` 三个不同 EP 学习且默认关闭 | SQLite 状态机、冲突撤销、WebUI、重匹配和 Episode worker tests |
...
| Docker/跨容器路径模型 | 下载器保存根、应用 download/save 根和路径/硬链接探测均显式配置 | 本机路径探测与生成的容器全链门禁；Docker 实跑 `[~]` |
...
- Docker amd64/arm64 构建、容器安全与完整容器 E2E。
- `win-arm64`、`linux-x64`、`linux-arm64`、`osx-arm64` 原生 runner 结果。
- 第一个实际 GitHub Prerelease 标签和 Release。
- 独立 AnimeGoNetData 仓库的 token/变量及首次不可变 Release。

这些项目不是缺少实现；对应 workflow、脚本和失败门禁已经提交。只有取得外部执行结果
后才能从 `[~]` 改为已验证。
