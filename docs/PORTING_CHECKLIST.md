...
| AI 季度/EP 匹配 | 独立默认关闭、600 秒超时 | 扩展 | 进行中（任务级契约、OpenAI-compatible HTTP、本地 MCP、TMDB 二次验证和后置 EP-AI 已完成；季度 AI 与 Mikan pubDate 预计算待实现） | fake AI/MCP/TMDB 已覆盖 EP 成功、字幕、顺序/Season 0/重复目标/身份越界/429/认证/网络失败/人工规则抑制；后续季度处理器 + NativeAOT 端到端 smoke |
