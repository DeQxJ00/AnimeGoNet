...
| RSS 黑白名单→有序规则组 | `MikanRssRuleEngine` | 扩展 | 进行中 | blacklist-first、whitelist、lowercase、旁路、短路和 stable-order tests 已通过；schema v13 store 与 GET/PUT/preview API 已验证 revision、禁用旁路和决策详情，RSS XML/批次编排与 WebUI 待实现 |
