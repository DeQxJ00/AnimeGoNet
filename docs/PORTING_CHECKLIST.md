...
| `mikan_tool.py` `Filiter0..4` | 内置 C# MikanTool | 替换 | 进行中 | pure differential + schema v15 规则/快照 + legacy config API + Episode identity parser + schema v16 逐候选 filter audit/优选隔离已验证；安全页面抓取与真实 RSS 执行待实现 |
