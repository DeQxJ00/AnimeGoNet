...
| Bangumi archive/cache | SQLite-backed archive refresh | 替换存储 | 已验证 | 官方 `aux/latest.json`/ZIP SHA 门禁、AOT-compatible DataBuilder 的动画/正片筛选、文本/日期/小数 EP 规范化、Subject 范围分片、确定性 JSONL.gz/manifest/离线 ZIP、原子输出及真实 SQLite 导入已验证；版本化下载/导入、保留/回滚和活动版本 read-through 完整串联，Actions 每周/手动构建但不自动发布 |
