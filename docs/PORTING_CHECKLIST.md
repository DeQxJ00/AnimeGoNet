...
| AI 元数据匹配 | 单开关、单Prompt、每任务最多一次、默认关闭、600 秒超时 | 扩展 | 已验证 | fake AI/MCP/TMDB 已覆盖关闭零请求、未配置、超时、429 重试/耗尽、认证、外层/模型畸形 JSON、多个 choices 歧义、不存在 ID、文件身份冲突、MCP schema 缓存，以及 Skip→Backtrace→AI→Title→First、阶段间禁止二次调用、EP/字幕/Other、跨季度、Season 0/重复目标/身份越界和 31 天日期窗口；AniDB 零参数固定公网连接与 IMDb external find 的 Movie 程序侧剔除已验证；五 RID NativeAOT workflow 运行发布二进制 loopback smoke，真实后台 worker 完成 fake AI 两轮→MCP 工具→fake TMDB Series/Season/Episode 验证→SQLite/API 权威状态落库，win-x64 本机已通过 |
