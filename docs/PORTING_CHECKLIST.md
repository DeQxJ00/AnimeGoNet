...
| `internal/constant`、`exceptions` | 强类型常量、稳定错误码 | 保留 | 已验证 | 上游状态/通知/插件/缓存/目录 sidecar 常量由所属边界的闭合 enum、record 与 compile-time 常量承载；集中 `StableErrorCode` 对所有跨 Core/Data/App 的持久化或投影错误码执行 1–128 位 ASCII 标识校验，Torrent 格式异常也提供稳定码；domain + Data/App integration tests |
...
| 上游路径/行为 | AnimeGoNet 目标 | 类型 | 状态 | 验收证据 |
|---|---|---:|---:|---|
| `internal/pkg/request` | AOT-safe `HttpClient` pipeline | 保留 | 已验证 | 真实 loopback socket已验证固定已校验 IP 连接、原 URI Host、固定 User-Agent、禁止自动 redirect 和流式响应；生产 SSRF 策略继续拒绝 loopback/private。TMDB/Bangumi 独立 API 地址、HTTP(S)/SOCKS5 代理、逐次超时、可配置额外重试/间隔、请求重建、取消传播及 429/5xx 与非重试错误边界已通过故障注入。Mikan SourceProfile 私有 Cookie 在 RSS/Torrent 请求中只发往原始 Host，跨 Host redirect 剥离；API/WebUI/日志格式化不回显。全部通过 loopback/API/WebUI/NativeAOT smoke |
...
| TMDB 季度失败链 | `TMDBFailSkip=4`→`TMDBFailBacktrace=3`→`TMDBFailUseTitleSeason=2`→`TMDBFailUseFirstSeason=1` | 扩展 | 进行中 | 日文名→中文名及每名多轮清理均以完整 `tmdbid+Season` 为成功条件；P3 对每个前作重新联合搜索且可恢复不同 Series，无前传/多层/同层多候选/缺日期/回溯到底/防环/Bangumi与TMDB失败/取消等确定性 fixture 已验证；P2 只读任务 title、P1 本地 S01 且均不验证 TMDB Season；关系网络重试与受控 live fixture 待实现 |
| AI 元数据匹配 | 单开关、单Prompt、每任务最多一次、默认关闭、600 秒超时 | 扩展 | 进行中（任务级契约、OpenAI-compatible HTTP、本地 MCP、TMDB 二次验证、统一 Series/Season/Episode 流程、Mikan pubDate/Bangumi EP 门控、跨季度逐文件状态、固定 AniDB 与 IMDb→TMDB TV 参考工具已完成） | fake AI/MCP/TMDB 已覆盖关闭零请求、未配置、超时、429 重试/耗尽、认证、外层/模型畸形 JSON、多个 choices 歧义、不存在 ID、文件身份冲突、MCP schema 缓存，以及 Skip→Backtrace→AI→Title→First、阶段间禁止二次调用、EP/字幕/Other、跨季度、Season 0/重复目标/身份越界和 31 天日期窗口；AniDB 零参数固定公网连接与 IMDb external find 的 Movie 程序侧剔除已验证；后续发布二进制 fake AI HTTP 端到端 smoke |
