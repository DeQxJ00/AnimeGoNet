...
| TMDB 完全失败记录 | `tmdbid=0` + bgmid + 待补全 | 扩展 | 进行中 | 权威 SemanticNoMatch 白名单、AI 优先恢复、有效 bgmid/季度门禁、`anime_series(tmdbid=0)`、无伪造 EP 的 Other 整理、根级 NFO 与 fallback completion 已验证；下载前 fallback claim/重复早停、恢复合并和待补全专用 UI 待实现 |
