...
| 特别篇/小数 EP | 已知季度 `Other`，不伪造整数 EP | 扩展 | 已验证 | 48.5 与 SP/OVA/OAD/PV/NCOP/NCED/Menu/S00 已阻止形成普通整数候选；Season 确认后小数集、特别篇、普通 TMDB EP 缺失、AI 未匹配及孤立字幕均持久化稳定 Other 原因，保留原名移动到 `Sxx/Extras`，且不写 Episode completion/alias/claim 或 `Eyyy.e_json` 伪进度 |
