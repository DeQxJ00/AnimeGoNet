...
| AI 元数据匹配 | 单开关、单Prompt、每任务最多一次、默认关闭、600 秒超时 | 扩展 | 进行中（任务级契约、OpenAI-compatible HTTP、本地 MCP、TMDB 二次验证、统一 Series/Season/Episode 流程、Mikan pubDate/Bangumi EP 门控、跨季度逐文件状态、固定 AniDB 与 IMDb→TMDB TV 参考工具已完成） | fake AI/MCP/TMDB 已覆盖 Skip→Backtrace→AI→Title→First、阶段间禁止二次调用、EP/字幕/Other、跨季度、顺序/Season 0/重复目标/身份越界/429/认证/网络失败、31 天日期窗口；AniDB 零参数固定公网连接与 IMDb external find 的 Movie 程序侧剔除已验证；后续发布二进制 fake AI HTTP 端到端 smoke |
