...
| `configs/utils.go` 环境变量 | 部署配置环境变量覆盖 | 保留 | 进行中 | 上游 `ANIMEGO_WEB_HOST/PORT`、原生/Docker 安全默认和 `--urls`/`ASPNETCORE_URLS` 优先级已有真实 Kestrel tests；旧 `ANIMEGO_PROXY_URL` 已保持全局双客户端语义、专用变量优先、空值关闭和 TMDB/Bangumi 双字段锁；当前应用配置及每个下载器实例的环境/命令行字段均投影为 `locked_fields`，Web 逐字段只读且 API 拒绝改写；下载器锁覆盖 type/base URL/用户名/密码/download path/enabled，保存其他字段不会复制环境凭据或固化环境值；路径、旧全局 tag 及完整旧配置 precedence 待实现 |
