...
范围说明：U2 当前交付 `inner_plugin_u2` 手动按钮链。矩阵中已有的 U2 路由用例锁定
...
| Web UI 作品库 | TMDB 名称/Cover/Season、EP完成网格、五种稳定排序、外部媒体手动补录、待补全TMDB | U/C/I/E | EP全集与状态只来自TMDB和规范完成记录；最后 EP 变动只取规范完成记录的最新完成时间；外部扫描默认不运行且只补录标准唯一文件；排序/分页稳定；`tmdbid=0` 不伪造进度 |
...
5. Episode URL 仅在成功取得有效 `mikanid+groupid` 后写入 `bolt/mikan_episode_identity`；
   作品页仅在成功取得 bgmid 后写入 `bolt/mikan_bangumi_identity`。两者默认 8760 小时、
   可独立配置且 `0` 表示永久，并可跨 resolver/进程生命周期复用；失败不 negative cache，
   带查询参数 Episode URL 旁路长期缓存但仍正常解析。系统缓存可查看明文 JSON 并精确删除。
...
5. 修改配置，先验证预览无写入、字段级即时/重启效果、密钥明文 before/after；表单再变化时旧预览失效。明确确认后验证旧 revision 备份与新 revision 原子写入，恢复部署默认前也生成备份；构造同 revision 不同备份内容时必须拒绝覆盖当前配置，并验证日志与运行状态仍未包含密钥。
6. 修改插件 args/vars，验证校验失败不会落盘。
7. 从一级“日志”菜单查看运行日志，组合验证级别/业务域/时间/异常筛选、暂停/恢复和 WebSocket 断线重连；再按阶段、结果、模型、时间筛选 AI 调用日志，核对跨任务分页汇总与单条 Token/HTTP/工具调用，确认 Prompt、响应正文和凭据均未返回。
...
20. 分别按最后更新日期、最后 EP 变动时间、TMDB 名称、TMDB Season 开播日期和加入日期升/降序分页；验证无 EP 完成记录与空开播日期置后、同值稳定排序、翻页无重复漏项，默认最后更新日期降序。
