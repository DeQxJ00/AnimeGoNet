...
WebUI 在 `:root` 明确使用 `16px` 作为 `rem` 基准，避免 Chrome“字号”、Firefox 与 Edge 的默认字体尺寸差异改变整套布局。浏览器或操作系统的无障碍“最小字号”仍具有更高优先级，程序不使用缩放技巧绕过用户的可访问性设置。

一级菜单“输入源”统一承载来源管理、Mikan 和 U2 功能；二级菜单显示“输入源管理”、
“Mikan”和“U2”。进入“Mikan”后展开三级菜单：手动设置、人工规则、可信 Offset、
候选规则和五级过滤；进入“U2”后可查看插件调用日志。

“输入源管理”提供独立的“新增 Mikan 来源”和“新增 U2 来源”入口。U2 模板默认填入
`u2` 稳定 ID、`pt` 下载器、`link` 文件策略、`animegonet-u2` 分类、`u2` Tag、
无限做种以及 `u2.dmhy.org` Host 白名单；用户仍须按实际环境确认下载器和策略后保存。
选择 U2 时隐藏 Mikan Cookie、RSS、Cron、旧过滤和同集优选字段，并显示内部插件设置与
U2 调用日志快捷入口。油猴脚本逐次人工确认的 TV/Movie 优先于来源默认媒体类型。
...
`Extras`，不计入 Other 待处理。TV 任务中的文件名若含 `劇場版`、`剧场版`、不区分
大小写的 `Gekijouban` 或 `movie`，则保留为 Other，作为人工“TV+Movie 后处理”的候选入口；程序不会
...
总览“运行状态”将关键数字划分为“系统资源”“下载与整理”“匹配与人工处理”“资产与运行规模”四组，并每 10 秒从现有权威 API 独立刷新。“系统资源”固定放在统计区最上方，以三列宽卡显示 AnimeGoNet 主进程 Working Set、按逻辑 CPU 数归一化的主进程 CPU 占用，以及当前 `data_path`（通常为 `animegonet_data`）的文件容量；资源数值独占一行，标题和说明不会被长容量值挤压。数据目录容量扫描不跟随重解析点并缓存 60 秒，避免 10 秒总览刷新反复遍历磁盘。下载组显示活动、暂停、失败、等待整理、重复跳过、已完成及快照过期任务；`download_skipped_duplicate` 使用独立统计，不计入等待整理。匹配组显示 Other、匹配错误、等待人工审核及待补全 TMDB；规模组分别显示动画季度、动画电影、匹配任务、已启用来源和离线下载器，电影数量直接采用 Movie 作品库 API 的 `total_items`，不混入 TV 季度数。所有统计卡均为原生按钮，动画季度和动画电影分别直达 `#/library/seasons` 与 `#/library/movies`；运行内存与 CPU 进入运行日志，数据目录容量进入目录设置。下载与前三类匹配统计会清除冲突筛选并套用与目标列表相同的服务端筛选条件，其他统计进入相应管理页面。各类统计请求并行执行且允许部分成功，单个 API 失败只把对应数字标记为读取失败，不覆盖其他统计，也不在总览维护第二套业务状态。
...
作品库季度卡片的作品名固定保留两行高度，超过两行时由 CSS 省略，避免长短标题造成 Cover 卡片内容和进度条上下错位；鼠标停留在标题上时通过原生提示显示未经截断的完整 TMDB 规范名称，卡片的可访问名称也继续包含完整作品名与季度名。列表仍使用原有三列、1000px 以下两列、760px 以下单列的响应式规则。卡片 Cover 固定使用标准 `2:3` 画幅并按宽度计算高度，不再通过 `height: 100%` 随文字内容拉长；详情页海报保持原有独立布局。
...
同一页面的可信 Offset 黑名单可按整个 mikanid、整个 groupid、精确 mikanid + groupid 三种范围增删。加入时明确提示会清除命中范围已有的自动学习证据与缓存；黑名单不修改人工规则、任务、下载记录或媒体文件。
...
“缓存 / Bangumi缓存”页面同时显示本地缓存使用记录。一次 Subject 查询由活动档案直接返回时计入 Subject 命中；Episode 只有在本地档案已包含完整普通集集合时才计入；schema v2 前传关系（包括权威空集合）由本地返回时计入关系命中。在线 Bangumi 回源不计入。计数按数据版本原子累加并跨重启保留，页面汇总全部历史版本的累计命中和最近命中时间。

“缓存 / AniDB缓存”按可配置间隔（默认 24 小时，1–720 小时）检查 `https://anidb.net/api/anime-titles.xml.gz`，并支持显式手动刷新。程序保留原始 gzip 到 `data_path/cache/anidb/anime-titles.xml.gz`，同时用流式 XML 读取把 AID、语言、标题类型和明文标题写入 SQLite。只有完整下载、gzip/XML 校验和数据库事务全部成功后才替换当前索引；失败继续使用上一版。页面显示来源、本地文件、更新时间、下次检查、动画数、标题数、压缩包大小和稳定失败码，并支持按 AID 或规范化标题分页查询。
...
“总览、动画库、任务中心、输入源、下载工具配置、插件、设置与备份、AI 匹配测试、通知、日志、缓存”；“动画库”提供“作品与季度 / 动画电影 / 待补全 TMDB”二级入口，电影列表独立读取 Movie 作品库 API；“AI 匹配测试”提供“AI 元数据测试 / AI 字幕匹配”二级入口；缓存工作区提供“Bangumi缓存 / AniDB缓存 / 其他缓存管理”，AnimeGoNetData 的活动/上一版本、在线更新、离线导入和回滚归入 Bangumi，AniDB 官方标题库归入 AniDB，通用 `bolt/themoviedb` 归入其他缓存管理；Mikan 的功能统一放在“输入源 / Mikan”三级菜单；现有 qBittorrent 实例管理归入“下载工具配置”，总配置归档归入“设置与备份 / 导入导出与备份”，不复制第二套下载器或应用配置页面；每个工作区在内容头部
...
稳定错误码、安全原因和独立的“AI 触发原因”，并可跳到对应任务。触发原因描述调用 AI 之前
哪一步确定性匹配仍未解决：季度阶段形如 `season_unresolved:<失败码>`，Episode 阶段形如
`episode_unresolved:<一个或多个文件原因>`；它不等同于模型返回结果或调用后的错误。schema v55
开始持久化该字段，历史调用无法可靠反推，统一明确显示“历史调用未记录”。只有 `ai_model`
非空、即 provider 确实发出请求的
...
“缓存 / 其他缓存管理”展示 SQLite `cache_buckets/cache_entries`。用户可在 `bolt` 与
...
字幕压缩包导入：季度详情提供“导入字幕压缩包”，支持 ZIP、RAR、7z、TAR 以及 GZ/BZ2/XZ 压缩包（包括 `.tgz`、`.tbz2`、`.txz`）。程序只提取字幕扩展名，先按文件名读取 EP 并默认勾选可识别项；用户可取消、修改集号或拖拽排序后确认导入。普通字幕按 `E###.<扩展名>` 写入季度，无法确认的文件进入 `Extras`。单个文件、解压后总大小和文件数量仍受安全上限限制。独立“AI 匹配字幕”按钮使用 `subtitle-v1` 专用提示词，AI 结果仍需人工复核。
