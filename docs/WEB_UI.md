...
“总览、动画库、任务中心、输入源、Bangumi缓存、下载工具配置、插件、设置与备份、AI 匹配测试工具、通知、日志、系统缓存”；AnimeGoNetData 的活动/上一版本、在线更新、离线导入和回滚统一归入“Bangumi缓存”，通用 `bolt/themoviedb` 缓存仍留在“系统缓存 / 缓存管理”；Mikan 的五项功能统一放在“输入源 / Mikan”三级菜单；现有 qBittorrent 实例管理归入“下载工具配置”，总配置归档归入“设置与备份 / 导入导出与备份”，不复制第二套下载器或应用配置页面；每个工作区在内容头部
