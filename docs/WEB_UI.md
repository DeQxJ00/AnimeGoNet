...
总览“运行状态”将关键数字划分为“下载与整理”“匹配与人工处理”“资产与运行规模”三组，并每 10 秒从现有权威 API 独立刷新。下载组显示活动、暂停、失败、等待整理、重复跳过、已完成及快照过期任务；`download_skipped_duplicate` 使用独立统计，不计入等待整理。匹配组显示 Other、匹配错误、等待人工审核及待补全 TMDB；规模组显示动画季度、匹配任务、已启用来源、离线下载器、AnimeGoNet 主进程 Working Set、按逻辑 CPU 数归一化的主进程 CPU 占用，以及当前 `data_path`（通常为 `animegonet_data`）的文件容量。数据目录容量扫描不跟随重解析点并缓存 60 秒，避免 10 秒总览刷新反复遍历磁盘。所有统计卡均为原生按钮，点击后直接打开对应页面；运行内存与 CPU 进入运行日志，数据目录容量进入目录设置。下载与前三类匹配统计会清除冲突筛选并套用与目标列表相同的服务端筛选条件，其他统计进入相应管理页面。六类统计请求并行执行且允许部分成功，单个 API 失败只把对应数字标记为读取失败，不覆盖其他统计，也不在总览维护第二套业务状态。
