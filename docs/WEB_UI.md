...
“总览、动画库、任务中心、输入源、下载工具配置、插件、设置与备份、AI 匹配测试工具、通知、日志、缓存”；“动画库”提供“作品与季度 / 动画电影 / 待补全 TMDB”二级入口，电影列表独立读取 Movie 作品库 API；缓存工作区提供“Bangumi缓存 / AniDB缓存 / 其他缓存管理”，AnimeGoNetData 的活动/上一版本、在线更新、离线导入和回滚归入 Bangumi，AniDB 官方标题库归入 AniDB，通用 `bolt/themoviedb` 归入其他缓存管理；Mikan 的功能统一放在“输入源 / Mikan”三级菜单；现有 qBittorrent 实例管理归入“下载工具配置”，总配置归档归入“设置与备份 / 导入导出与备份”，不复制第二套下载器或应用配置页面；每个工作区在内容头部
...
字幕压缩包导入：季度详情提供“导入字幕压缩包”。ZIP 只提取字幕扩展名，程序先按文件名读取 EP 并默认勾选可识别项；用户可取消、修改集号或拖拽排序后确认导入。普通字幕按 `E###.<扩展名>` 写入季度，无法确认的文件进入 `Extras`。独立“AI 匹配字幕”按钮使用 `subtitle-v1` 专用提示词，AI 结果仍需人工复核。
