...
汇总面板下方提供逐条命中明细，每条显示命中类型、Bangumi Subject ID（bgmid）、返回记录数、AnimeGoNetData 版本、命中时间与稳定序号；默认最新优先，可按 Subject、Episode 集、前传关系筛选，并以 25/50/100 条服务端分页。权威空关系集合会明确记录为返回 0 条。schema v46 启用之前没有足够证据恢复逐条历史，因此不推测补造；汇总面板的既有累计数仍保留。
