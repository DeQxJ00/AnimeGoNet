...
作品库季度卡片的作品名固定保留两行高度，超过两行时由 CSS 省略，避免长短标题造成 Cover 卡片内容和进度条上下错位；鼠标停留在标题上时通过原生提示显示未经截断的完整 TMDB 规范名称，卡片的可访问名称也继续包含完整作品名与季度名。宽屏保持三列；视口低于 1500px 时切换为两列，避免侧栏占用空间后仍强制三列而让 Cover 卡片显得过窄；760px 以下再切换为单列。
