...
- 活动下载、暂停、死种、卡住、等待整理、整理失败和完成任务数量。
...
- 暂停、恢复 qBittorrent 任务；命令幂等并在下一次同步确认实际状态。已进入下载队列的任务从下发时间开始计算，连续 `7×24h` 同时保持 `progress=0` 和 `downloaded_bytes=0` 时标记为独立的 `dead`（死种）状态并自动暂停。死种不同于普通人工暂停，WebUI 使用黄色卡片和徽章展示并可独立筛选；人工恢复后以成功的恢复事件作为新观察窗起点，仍然需要再次连续满 `7×24h` 零进度才会重新判定。已有任何下载字节或非零进度的任务不进入该规则。
...
- Web 通过版本化查询 API 读取 AnimeGoNet 缓存；下载任务二级页面可见时约 5 秒刷新，切换到其他一级/二级页面或浏览器隐藏后停止轮询。内容未变化、详情展开、编辑控件聚焦或对话框打开时保留现有 DOM；安全更新列表时保持首个可见任务卡片的视口位置。
...
- `GET /api/v1/downloads`：接受 `page`、`page_size`、`search`、qB `state`、AnimeGoNet `business_status`、`downloader_id`、`source` 和摘要快捷筛选 `summary_bucket`，筛选与分页在 SQLite 查询中执行。`summary_bucket` 支持 `active`、`paused`、`failed`、`waiting_organization`、`skipped_duplicate`、`completed`、`stale`，并与仪表盘聚合复用完全相同的业务谓词。默认按任务加入时间倒序稳定排序，也可选择最后更新时间或异常与活动优先。
