...
唯一正式 Prompt 见 [`TMDB_AI_MATCH_PROMPT.md`](TMDB_AI_MATCH_PROMPT.md)。本次契约版本为 `tmdb-ai-match-v13`；实现不得维护第二份运行时 Prompt。部署配置或私有配置可以覆盖模板正文，但必须保留契约要求的全部占位符和条件区块；变更内置模板时更新 `prompt_version` 并通过 snapshot review。
