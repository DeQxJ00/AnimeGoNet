...
- [x] 将已验证独立 AI Tester 1:1 内置到主程序：保留原 `UiRunRequest/UiRunResponse` 字段、Responses/Chat 请求格式、最多 8 轮工具调用、stateful→stateless 续轮、用量累计、`request_identity`、结果 JSON 校验、Mikan pubDate 门禁、本地文件 EP 候选/offset、完整 AI 请求审计与工具 Request/Response Content；提供 `/api/v1/ai-test/run-stream` NDJSON、按 `run_id` 停止、可信 Torrent `import_id` 和 Mikan Episode 导入，并兼容原 `/api/run*`/`/api/import-*` 路径。WebUI 只调整为主程序侧栏样式与分区布局，不再简化协议。安全差异明确保留：API Key 不持久化/不回显，passkey Torrent URL 不返回，Tester 结构校验之后额外显示主程序 TMDB Series/Season/Episode 二次验证。46 项 API/协议 parity 测试通过；真实模型/MCP 浏览器验收待本地配置后执行。
