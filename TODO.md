...
- [x] 将成功解析的 Mikan Episode URL → `mikanid+groupid` 升级为 SQLite 长期缓存：无自动过期，跨刷新/重启复用；聚合 RSS、legacy 过滤和 AI 测试工具的 Mikan URL 导入统一命中。只有两个 ID 都有效时写入，失败、网络异常、缺 groupid 和带查询参数 URL 不做缓存；`bolt/mikan_episode_identity` 可在“系统缓存”逐条查看和手动删除，不保存页面 HTML、Cookie、Torrent URL 或 passkey。
