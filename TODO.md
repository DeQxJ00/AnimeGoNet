...
- [>] 实现作品库季度列表：schema v23 已持久化 TMDB Series/Season 名称、首播日期、总集数与 Series/Season poster 路径；P4/P3 联合匹配会再请求官方 Season endpoint，并在正常解析和待补全恢复事务中保存完整普通 Episode snapshot。`GET /api/v1/library/seasons` 已按 `TMDB Series + 普通 Season` 批量聚合正式作品、snapshot、规范完成记录、解析来源和一致性警告；`GET /api/v1/library/seasons/{tmdbSeriesId}/{seasonNumber}` 已提供只认官方 snapshot 与规范完成记录的 EP 网格，删除完成记录会立即恢复未下载；Cover 安全代理/缓存/占位图已完成。前端页面待实现。
