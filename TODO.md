...
- [x] 主程序加入只读 AI 测试链：`POST /api/v1/ai-test/run` 与 WebUI“测试工具 / AI 元数据测试”复用生产 Prompt、OpenAI-compatible matcher、MCP 工具和 TMDB 二次验证；显示最终原始输出、解析/验证、请求与工具阶段日志、Token、耗时及实际启用部分。支持测试请求级 Prompt 编辑/同版本浏览器草稿，并按开关与输入 ID 裁剪 TMDB/Bangumi/AniDB/IMDb/Mikan 日期条件区块且同步限制实际工具注册；模型地址、模型名、临时 Key、API 模式（Responses/Chat Completions）、超时、重试、MCP 地址和 HTTP 代理可仅覆盖本次测试，Key 不保存、不回显、不写日志。也可从任意受支持 Mikan Episode URL 安全读取页面、精确 RSS item、bgmid 和 Torrent 文件后填表。上述操作都不创建下载/元数据任务、不写动画库；Mikan URL 导入继续服从主程序全局域名代理规则。
