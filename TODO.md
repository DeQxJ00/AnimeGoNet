...
- [>] 移植环境变量覆盖：规范嵌套键、现有扁平键、旧 qB 兼容键及命令行优先级已接入；上游 `ANIMEGO_WEB_HOST/PORT`、原生 `127.0.0.1:7991`、Docker `0.0.0.0:7991` 和标准 `--urls`/`ASPNETCORE_URLS` 高优先级均有真实 Kestrel 监听测试；应用字段和下载器实例字段的部署锁均在 API/WebUI 逐字段显示来源，拒绝改写且保存其他字段时不把环境凭据复制进私有覆盖。路径、旧全局 proxy/tag 及完整旧配置 precedence parity 仍待补齐。
