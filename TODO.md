...
- [x] 将外部插件/API 与 WebUI 鉴权完全拆分：`web.access_key` 仅保护 AnimeGoHelper、兼容插件接口和精确的统一导入端点，`web.webui_access_key` 独立保护其余管理 API/日志 WebSocket且默认留空；两把密钥使用不同 header/query、不能交叉授权，应用配置页可明文回填、保存备份并生成不含明文的 WebUI 专用地址。WebUI 使用的 Mikan URL 解析与 RSS 手动导入保持在 WebUI 边界。
