...
- [x] 一级菜单统一为“缓存”，下分“Bangumi缓存 / AniDB缓存 / 其他缓存管理”。Bangumi 页保留 AnimeGoNetData 离线 Subject、Episode、前传关系的版本状态、检查、下载、离线导入、回滚和逐条命中审计；通用 `bolt/themoviedb` 留在其他缓存管理。AniDB 页每天条件下载官方 `anime-titles.xml.gz`，流式验证后原子替换 SQLite 明文标题索引，支持按 AID/标题查询；下载或解析失败保留上一版。schema v68 保存 AniDB 更新状态、标题类型、语言和规范化检索字段。
...
- [x] 实现仪表盘和下载器/任务状态：下载状态卡片、进度、连接且非 stale 的跨实例速度汇总、活动/暂停/死种/失败/等待整理/完成/离线指标、qB 状态与 AnimeGoNet 业务阶段独立筛选，以及 `download_preparing`/重复跳过、元数据 Series/Season/Episode 阶段、失败原因、策略尝试时间线、文件归类计数、准备/整理失败详情和显式重试入口均已接入。已进入下载队列且从任务下发或最近一次死种恢复起连续 `7×24h` 保持 `progress=0`、`downloaded_bytes=0` 的任务会自动暂停并独立标记为黄色“死种”；人工恢复会开启新的完整 7 天观察窗。
...
- [x] 实现按实例隔离的qB同步器和`DownloaderTaskSnapshot`：活动约2秒、空闲约10秒、单实例单在途、实例失败隔离、离线保留stale快照、重启按实例+hash恢复，以及首错2秒、连续半开失败指数增长并封顶120秒的熔断已完成；显式连接测试可安全绕过等待窗并在成功后复位。同步器同时负责识别 7 天零字节/零进度死种并调用对应 qB 实例暂停，检测、暂停失败和人工恢复均进入持久化事件时间线。
...
- [x] 将 Episode AI 返回不同 TMDB Series 的情况从普通匹配失败中拆分为“多语言标题命中不同 TMDB Series / 疑似 TMDB 重复条目”：主程序不自动覆盖已确认 Series，而是对 AI 候选再次完整验证 Series/Season/Episode 并以 schema v60 保存单文件审核提议。任务整理到 Other 后，人工审核表格显示原 TMDB 与 AI 候选；用户可明确同意变更并复用安全重新整理，或拒绝并保留 Other。两种决定均留审计，同意后整理完成仍需最终审核；Prompt 未修改。
- [x] 对人工同意的 AI TMDB Series 变更建立精确 `mikanid + groupid` 长期映射：schema v61 在审核提议中固化 Mikan 身份，同意后保存目标 TMDB Series/Season；后续同组合在标题搜索前读取并重新验证 Series/Season，Episode 仍走正式匹配与 TMDB 验证。强制 Other 重新适配、缺少 ID、不同字幕组和 Movie 不复用；“输入源 → Mikan → 人工 TMDB 映射”可审计并删除单条映射，删除不改写历史结果。
...
- [x] U2 来源增加“优先使用 AniDB 映射 TMDB”开关：默认启用，按 AniDB 映射 → 官方 AniDB 标题缓存 → 正常标题/AI 的顺序尝试；映射得到的 Series/Season 与文件 EP 仍必须通过现有 TMDB 验证。AniDB 标题缓存检查间隔可在 WebUI 设置（1–720 小时）。
- [x] 动画库季度详情增加字幕压缩包导入：ZIP、RAR、7z、TAR 及 GZ/BZ2/XZ 包装的 TAR 中的字幕文件先用文件名解析 EP，WebUI 展示可勾选、手动修改和拖拽排序的匹配列表；确认后普通字幕写入 `E###.<扩展名>`，未匹配字幕写入 `Extras`。独立“AI 匹配字幕”按钮使用单独的 `subtitle-v1` 提示词，AI 结果仍需人工复核后导入；`subtitles-test/` 提供被 Git 忽略的真实三格式夹具，显式 LocalIntegration 测试见 `docs/SUBTITLE_ARCHIVE_LOCAL_INTEGRATION.md`。
- [x] WebUI 设置页增加重启按钮；仅调用 `IHostApplicationLifetime.StopApplication`，不会伪造守护进程重启，页面明确要求由 systemd/Docker/服务管理器负责拉起。
